...
static void nds32_elf_relocate_ex9_table (struct bfd_link_info *);
...
/* For ISPS.  */
static void nds32_elf_isps_init (char *name);
static void (*nds32_elf_isps_table_init) (void);
static bfd_boolean (*nds32_elf_isps_relocation)
  (bfd_vma, bfd_vma, bfd_boolean,
   bfd_byte *, bfd_byte *, unsigned short *);
static void (*nds32_elf_isps_get_contents)
  (bfd_vma (*) (const void *));
			      ...
			      struct elf_link_hash_entry *h ATTRIBUTE_UNUSED)
{
  ...
  /* Do not export these ifc symbols, which are for the internal use only.  */
  if (h->root.root.string
      && strstr (h->root.root.string, "$nds32ifc_"))
    return TRUE;
			  ...
			  bfd_byte *contents)
{
  ...
  if (ex9_section && ex9_contents == NULL)
  ...
  if (!nds32_elf_isps_relocation (start_rel->r_offset, end_rel->r_offset,
				  ex_final, contents, ex9_contents, &sum))
...
static unsigned int
nds_fls (register unsigned int x)
{
  return ffs (x & (-x));
}
			    ...
			    asection **local_sections)
{
  ...
  if (table->target_optimize & NDS32_RELAX_EX9_ON)
    nds32_elf_relocate_ex9_table (info);
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	case R_NDS32_LO12S2_RELA:
	  /* alignment check  */
	  if ((relocation + rel->r_addend) & 3)
	    {
	      /* Incorrect alignment.  */
	      (*_bfd_error_handler)
		(_("%B(%A): warning: relocation is truncated of type %d @ 0x%08x."),
		 input_bfd, input_section, r_type, rel->r_offset);
	      /*ret = FALSE;
	      goto check_reloc;*/
	    }
...
handle_sda:
	  ...
	  if ((relocation + rel->r_addend) & align)
	    {
	      /* Incorrect alignment.  */
	      (*_bfd_error_handler)
	...
	case R_NDS32_TLS_IEGP_LO12S2:
	  {
	    ...
	    if (1 < ones32 (tls_type))
	      {
		eff_tls_type = 1 << (nds_fls (tls_type) - 1);
		...
		if (eff_tls_type == GOT_TLS_LE)
		  {
		    eff_tls_type = 1 << (nds_fls (tls_type ^ eff_tls_type) - 1);
		  }
	      }
	...
	case R_NDS32_SECURITY_16:
	  if (table->isps_file_name != NULL
	      && strlen (table->isps_file_name) != 0)
  ...
  if (r_symndx >= symtab_hdr->sh_info)
	  ...
	  || bfd_is_abs_section (sym_sec))
	{
	  ...
	  if (elf32_nds32_hash_entry (h)->offset_to_gp == 0)
	    elf32_nds32_hash_entry (h)->offset_to_gp
	      = offset_to_gp - irel->r_addend;
	  else if (abs (elf32_nds32_hash_entry (h)->offset_to_gp + irel->r_addend)
      ...
      if (NDS32_GUARD_SEC_P (sym_sec->flags))
	{
	  ...
	  if (elf32_nds32_local_gp_offset (abfd)[r_symndx] == 0)
	    elf32_nds32_local_gp_offset (abfd)[r_symndx]
	      = offset_to_gp - irel->r_addend;
	  else if (abs (elf32_nds32_local_gp_offset (abfd)[r_symndx] + irel->r_addend)
		   < abs (offset_to_gp) - sda_offset)
	    {
	      if (*access_addr >= local_sda)
		*access_addr += (record_count * 4);
		      ...
		      Elf_Internal_Shdr *symtab_hdr, bfd_boolean *again)
{
  ...
  nds32_elf_final_sda_base (sec->output_section->owner, link_info,
			    &local_sda, FALSE);
  access_addr = calculate_memory_address (abfd, irel, isymbuf, symtab_hdr);
  flsi_offset = (insn & 0xfff);
  if (flsi_offset & 0x800)
    flsi_offset = ~(bfd_vma)0xfff | flsi_offset;
  flsi_offset <<= 2;
		      ...
		      struct bfd_link_info *link_info)
{
  ...
  /* The first section reenter the function.  */
  static bfd_boolean set = FALSE;
  /* Enter this function first time.  */
  static bfd_boolean first = TRUE;
  static int pass = NDS32_RELAX_NONE_ROUND;
  ...
  if (first)
    {
	 ...
	 Decide the first round here.  */
      if (table->target_optimize & NDS32_RELAX_IFC_ON)
	pass = NDS32_RELAX_IFC_ROUND;
      else
	pass = NDS32_RELAX_NORMAL_ROUND;
  ...
  if (!init && final_sec == sec)
	  ...
	  if (!normal_again)
	    {
	      ...
	      if (table->target_optimize & NDS32_RELAX_IFC_ON)
		{
		  pass = NDS32_RELAX_JUMP_IFC_ROUND;
		  *again = TRUE;
		}
	      else
		goto do_ex9;
	...
	case NDS32_RELAX_EX9_BUILD_ROUND:
	  ...
	  pass = NDS32_RELAX_EX9_REPLACE_ROUND;
	  *again = TRUE;
	  break;
	case NDS32_RELAX_EX9_REPLACE_ROUND:
	  /* FIXME: Do not do jump ifc (nds32_elf_ifc_finish) again
	     here since it will cause some problems.  */

	  if (table->update_ex9_table && !ex9_replace_again)
	    {
	      ex9_replace_again = 1;
	      pass = NDS32_RELAX_EX9_BUILD_ROUND;
	      *again = TRUE;
	    }
	  break;
...
bfd_elf32_nds32_set_target_option (struct bfd_link_info *link_info,
				   ...
				   int relax_round, char *ex9_export_file,
				   ...
				   bfd_boolean ex9_ji_limit,
				   bfd_boolean ex9_noji,
				   bfd_boolean ex9_nols,
				   char *abi,
				   char *isps_file_name,
				   int keep_import_ex9)
{
  ...
  table->ex9_ji_limit = ex9_ji_limit;
  table->ex9_noji = ex9_noji;
  table->ex9_nols = ex9_nols;
  table->keep_import_ex9 = keep_import_ex9;
  output_abi = abi;
  table->isps_file_name = isps_file_name;

  /* We have to do ifc optimization before general relax.  */
  if (target_optimize & NDS32_RELAX_IFC_ON)
    {
      /* Initialize ifc hash table in nds32_elf_ifc_init. We will do it
	 for each input section in elf_nds32_ifc_trace_code later.  */
    }
  /* No need to build ex9 hash table when only ex9 import is setting.  */
  if ((target_optimize & NDS32_RELAX_EX9_ON)
      && (ex9_import_file == NULL || update_ex9_table))
    {
  ...
  if (table->isps_file_name != NULL
      && strlen (table->isps_file_name) != 0)
    nds32_elf_isps_init (table->isps_file_name);
}
				...
				bfd *abfd, int target_optimize)
{
  ...
  if (target_optimize & NDS32_RELAX_EX9_ON
    && (table->ex9_import_file == NULL
	|| table->keep_import_ex9
	|| table->update_ex9_table))
    {
...
/* After ex9 relaxation, the high 20 bits of symbol may be
   ...
   nds32_elf_ex9_replace_instruction, and then fixed the
   id of ex9.it insns at nds32_elf_relocate_ex9_table.  */
...
nds32_elf_order_insn_times (struct bfd_link_info *info)
{
  ...
  /* TODO: Only free the pointer, which point to the structure
     elf_nds32_insn_times_entry, is not enough. This may cause
     memory leakage.  */
  while (ex9_insn != NULL)
    {
...
nds32_elf_ex9_build_itable (struct bfd_link_info *link_info)
{
  ...
  if (table_sec != NULL)
	...
	return;
      ...
      if ((!table->ex9_import_file
	   || (table->ex9_import_file && !table->keep_import_ex9))
				    ...
				    FALSE, FALSE, TRUE))
	{
	 ...
	 into the nds32_elf_relocate_ex9_table.  */
      for (ptr = ex9_insn_head; ptr != NULL; ptr = ptr->next)
	{
	  long val;
...
nds32_elf_ex9_replace_instruction (struct bfd_link_info *info, bfd *abfd, asection *sec)
{
  ...
  while (off < sec->size)
      ...
      while (ex9_insn)
	{
	  ...
	  if (irel != NULL && irel < irelend && irel->r_offset == off
	      && ex9_insn->irel != NULL)
	    nds32_elf_get_insn_with_reg (&ex9_insn->rel_backup, it_insn,
					 &it_insn_with_reg);

	  /* Instruction and ex9 both have relocation.  */
	  if (insn_with_reg != 0 && it_insn_with_reg != 0
	      && (ELF32_R_TYPE (irel->r_info) ==
		  ELF32_R_TYPE (ex9_insn->rel_backup.r_info))
	  ...
	  /* TODO: I think that replacing the instruction with
	     relocation to ex9.it according to the imported ex9
	     table is a dangerous thing. Maybe we should review
	     this first when error occurring.  */
...
{
  struct elf_nds32_insn_times_entry *ex9_insn;
  struct elf_nds32_insn_times_entry *last_imported_entry = NULL;
  int target_optimize;
  struct elf_nds32_link_hash_table *table;
  ...
  struct elf_nds32_insn_times_entry *temp;
  /* TODO: Same as nds32_elf_order_insn_times, this may cause
     memory leakage.  */
  while (ex9_insn)
    {
			  ...
			  struct bfd_link_info *link_info)
{
  ...
  if (ptr->m_list != NULL)
	...
	*isec_ptr = *sec;
      ...
      relocation = nds32_elf_rela_local_sym (link_info->output_bfd, &sym,
					    &ptr->sec, ptr->irel);
      if (ptr->irel != NULL)
...
nds32_elf_ex9_import_table (struct bfd_link_info *info)
{
  ...
  /* We will run nds32_elf_ex9_build_itable for ex9 update
     in the nds32_elf_ex9_finish.  */
  if (!table->update_ex9_table && table->keep_import_ex9)
    nds32_elf_ex9_build_itable (info);
}
...
static void
nds32_elf_relocate_ex9_table (struct bfd_link_info *link_info)
{
				...
				struct bfd_link_info *link_info)
{
  ...
  while (off < sec->size)
      ...
      if (irel != NULL && irel < irelend && irel->r_offset == off)
	{
	  ...
	  shift = howto->rightshift;
	  align = (1 << shift) - 1;
	  if ((!table->ex9_noji && ELF32_R_TYPE (irel->r_info) == R_NDS32_25_PCREL_RELA)
	      || (!table->ex9_nols
	      ...
	      if (ELF32_R_TYPE (irel->r_info) == R_NDS32_25_PCREL_RELA)
		{
		  ...
		  insn_pc = sec->output_section->vma + sec->output_offset + off;
		  if ((relocation & 0xfe000000) != (insn_pc & 0xfe000000)
		      || (insn_pc > 0x2000000
			  && ((insn_pc & 0x1ff0000) == 0
			      || (relocation & 0x1ff0000) == 0)))
		    {
		      off += 4;
		      continue;
		    }

		  if (table->ex9_ji_limit
		      && (relocation >= 0x1000000
			  || insn_pc >= 0x1000000))
		    {
		      off += 4;
		      continue;
...
/* Ifc global symbol.  */
struct bfd_hash_table ifc_code_table ATTRIBUTE_UNUSED;
/* Since we can not do ifc cross sections so far, we store the information
   of ifc for each input section independently.  */
struct elf_nds32_ifc_info
{
  asection *sec;
  struct bfd_hash_table ifc_code_table;
  struct elf_nds32_ifc_code_hash_entry *ifc_insn_head;
  struct elf_nds32_ifc_info *next;
};
struct elf_nds32_ifc_info *ifc_info_head = NULL;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	  ...
	  && isym[ELF32_R_SYM (irel->r_info)].st_shndx == sec_shndx)
	{
	  ...
	  switch (ELF32_R_TYPE (irel->r_info))
	    ...
	    case R_NDS32_DIFF8:
	      offset = bfd_get_8 (abfd, contents + irel->r_offset);
	      nds32_elf_ifc_insert_barrier (irel->r_addend + offset, barrier);
	      break;
	    case R_NDS32_DIFF16:
	      offset = bfd_get_16 (abfd, contents + irel->r_offset);
	      nds32_elf_ifc_insert_barrier (irel->r_addend + offset, barrier);
	      break;
	    case R_NDS32_DIFF32:
	      offset = bfd_get_32 (abfd, contents + irel->r_offset);
	      nds32_elf_ifc_insert_barrier (irel->r_addend + offset, barrier);
	      break;
	    default:
	      break;
			  ...
			  bfd *abfd, asection *sec)
{
  ...
  /* Initialize the information of ifc for each input section.  */
  struct elf_nds32_ifc_info *ifc_info, *ifc_info_pre = NULL;
  ifc_info = ifc_info_head;
  while (ifc_info && ifc_info->sec && ifc_info->sec != sec)
    {
      ifc_info_pre = ifc_info;
      ifc_info = ifc_info->next;
    }
  /* The information of ifc has not been set for the section sec.  */
  if (!ifc_info)
    {
      ifc_info = bfd_malloc (sizeof (struct elf_nds32_ifc_info));
      ifc_info->sec = sec;
      ifc_info->ifc_insn_head = NULL;
      ifc_info->next = NULL;
      /* Init hash table for the section.  */
      if (!bfd_hash_table_init (&(ifc_info->ifc_code_table),
				nds32_elf_ifc_code_hash_newfunc,
				sizeof (struct elf_nds32_ifc_code_hash_entry)))
	{
	  (*_bfd_error_handler) (_("Ld error: cannot init ifc hash table "
				   "for the section: %s\n"), sec->name);
      ...
      if (!ifc_info_head && !ifc_info_pre)
	ifc_info_head = ifc_info;
      else
	ifc_info_pre->next = ifc_info;
    }
  ...
  nds32_elf_ifc_build_barrier (abfd, sec, &barrier, contents);
  ...
  while (off < sec->size)
	...
	bfd_hash_lookup (&(ifc_info->ifc_code_table), code, TRUE, TRUE);
      if (entry == NULL)
	{
	  (*_bfd_error_handler)
...
/* IFC hash table traverse function for all sections.  */
...
{
  unsigned int i;
  struct elf_nds32_ifc_info *ifc_info = ifc_info_head; 

  while (ifc_info)
    {
      ifc_info->ifc_code_table.frozen = 1;
      for (i = 0; i < ifc_info->ifc_code_table.size; i++)
	{
	  struct bfd_hash_entry *p;

	  for (p = ifc_info->ifc_code_table.table[i]; p != NULL; p = p->next)
	    func ((struct elf_nds32_ifc_code_hash_entry *) p,
		  &(ifc_info->ifc_insn_head));
	}
      ifc_info->ifc_code_table.frozen = 0;
      ifc_info = ifc_info->next;
    }
}
...
/* Used to check the unsigned long wrap for DIFF relocations.  */

static bfd_boolean
nds32_elf_diff_reloc_overflow (unsigned long a,
			       unsigned long b,
			       unsigned long max,
			       unsigned long mask)
{
...
nds32_elf_ifc_enlarge (bfd *abfd, asection *sec, bfd_vma off, int size)
{
  ...
  const char *error_msg =
    N_("\n*** Your code may trigger copy propagation bug."
       "\n*** Suggest upgrading your toolchain."
       "\n%B:(%pA+0x%x): The distance 0x%x of %s is overflow "
       "after enlarging the IFC basic pattern.  Please "
       "disable the IFC opt or jump table when compiling "
       "the file by the compiler option -mno-ifc or "
       "-fno-jump-tables.");
  ...
  /* Adjust r_osset and r_addend for all related relocations.  */
  for (sect = abfd->sections; sect != NULL; sect = sect->next)
    {
	...
	continue;
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  /* Adjust the r_offset of relocations.  */
	  if (sec == sect)
	    {
	  ...
	  /* DIFF relocation is used to calculate the distance between
	     two symbols in the same section, and is used for jump table,
	     and debug info. We have to adjust DIFF relocs carefully.

	     * .byte/.short/.word L1 - L2 -> R_NDS32_DIFF8/16/32
	     * r_offset: Where is the DIFF reloc need to be adjusted.
	       Arealy fixed above.
	     * r_addend: symbol (stored in r_info) + r_addend = L1.
	       It will be fixed later.
	     * The distance (L1 - L2, stored in "contents + r_offset")
	       need to be adjusted, too. Fix it now.  */

	  if (ELF32_R_TYPE (irel->r_info) >= R_NDS32_DIFF8
	      && ELF32_R_TYPE (irel->r_info) <= R_NDS32_DIFF32
	      && isym[ELF32_R_SYM (irel->r_info)].st_shndx == sec_shndx)
	    {
	      unsigned long val, mask = 0, offset;
		  ...
		  || (off <= irel->r_addend && off > irel->r_addend + val))
		{
		  ...
		  if (mask)
		    {
		      (*_bfd_error_handler)
			(_("ld error: ifc relaxation error."
			   "Please turn off ifc optimization or update toolchain.\n"));
		      abort ();
		    }
#endif

		  offset = mask ? val - size : val + size;
		  /* For ticket-13763, the distance of DIFF relocs may
		     be overflow after enlarging the IFC basic pattern.  */
		  /* TODO: fix the overflow problem rather than just print
		     the error msg.  */
		  switch (ELF32_R_TYPE (irel->r_info))
		    {
		    case R_NDS32_DIFF8:
		      if (!nds32_elf_diff_reloc_overflow
			  (val, size, 0xff, mask))
			{
			  (*_bfd_error_handler)
			    (error_msg, abfd, sect, irel->r_offset,
			     offset, "R_NDS32_DIFF8");
			  return;
			}
		      bfd_put_8 (abfd, offset, contents + irel->r_offset);
		      break;
		    case R_NDS32_DIFF16:
		      if (!nds32_elf_diff_reloc_overflow
			  (val, size, 0xffff, mask))
			{
			  (*_bfd_error_handler)
			    (error_msg, abfd, sect, irel->r_offset,
			     offset, "R_NDS32_DIFF16");
			  return;
			}
		      bfd_put_16 (abfd, offset, contents + irel->r_offset);
		      break;
		    case R_NDS32_DIFF32:
		      if (!nds32_elf_diff_reloc_overflow
			  (val, size, 0xffffffff, mask))
			{
			  (*_bfd_error_handler)
			    (error_msg, abfd, sect, irel->r_offset,
			     offset, "R_NDS32_DIFF32");
			  return;

			}
		      bfd_put_32 (abfd, offset, contents + irel->r_offset);
		      break;
		    }
		}
	    }
	  ...
	  if (ELF32_R_SYM (irel->r_info) < symtab_hdr->sh_info
	      && isym[ELF32_R_SYM (irel->r_info)].st_shndx == sec_shndx
	      && (ELF_ST_TYPE (isym[ELF32_R_SYM (irel->r_info)].st_info)
		  == STT_SECTION))
	    {
	      if (irel->r_addend <= sec->size
		  && irel->r_addend >= off)
  ...
  for (isymend = isym + symtab_hdr->sh_info; isym < isymend; isym++)
      ...
      if (isym->st_shndx == sec_shndx)
	{
	  if (isym->st_value <= sec->size)
		...
		isym->st_value += size;
	      /* Adjust function size.  */
	      else if (ELF32_ST_TYPE (isym->st_info) == STT_FUNC
		       && isym->st_size > 0
			      ...
			      struct bfd_link_info *info)
{
  ...
  /* Handle the basic IFC patterns, we have to insert "ifret" insn at
     the end of the pattern. Since it will enlarge the specific region
     of the code, we have to adjust the related relocations and symbols.

     Furthermore, insert a global symbol for the basic pattern is
     necessary, we will do it later when handling the other patterns
     relaxed to ifcall.  */
  ...
  /* Other patterns have to be relaxed to ifcall, and then insert
     the relocation (jump to the basic ifc pattern).  */
  ptr = stack;
  while (ptr)
    {
      if (ptr->live && ptr != base)
	{
	  ...
	  /* The larget range 16s and right shift one bit.
	     Do it conservatively, so pick range 0xf000.  */
	  ...
	  /* The first instruction offset.  */
	  off = member->offset - member->unit_p->entry_p->size;

	  /* Get the total instruntion size of the common subexpression.  */
	  ...
	  if (!bh)
		...
		- base_imember->member->unit_p->entry_p->size;
	      /* Insert a global symbol for ifc target (for basic ifc).  */
...
/* Find cse block here for all sections.  */
static void
  ...
  while (ifc_info)
    {
	 ...
	 we will know the max appearance times is 2, and the insntruction is C.  */
      struct elf_nds32_ifc_code_hash_entry *ptr = ifc_info->ifc_insn_head;
      struct elf_nds32_ifc_insn_stack *stack;
...
/* Initialization of ISPS.  */

static void
nds32_elf_isps_init (char *name ATTRIBUTE_UNUSED)
{
#ifndef __MINGW32__
  void *handle = NULL;
  ...
  nds32_elf_isps_table_init = dlsym (handle, "nds32_elf_isps_table_init");
  nds32_elf_isps_relocation = dlsym (handle, "nds32_elf_isps_relocation");
  nds32_elf_isps_get_contents = dlsym (handle, "nds32_elf_isps_get_contents");

  nds32_elf_isps_table_init ();
  nds32_elf_isps_get_contents (bfd_getb32);
#endif
}

