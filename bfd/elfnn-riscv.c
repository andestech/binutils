...
riscv_elf_size_dynamic_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  if (ict_model == 0xffffffff || !jvt_sym
      || jvt_sym->root.type == bfd_link_hash_undefined)
    {
...
typedef struct relo_expr
{
  Elf_Internal_Rela *end;
  bfd_vma expression;
  bool is_expression;
} relo_expr_t;
static relo_expr_t reloexpr;
		    ...
		    bfd_byte *contents)
{
  ...
  {
    const Elf_Internal_Rela *next = rel + 1;
    is_expression = (next < reloexpr.end) && (next->r_offset == rel->r_offset);
  }
  ...
  reloexpr.is_expression = is_expression && is_expression_op;
  reloexpr.expression = value;

  if (execit.htab->andes.set_truncation_check
      && !reloexpr.is_expression
      && value & ~howto->dst_mask)
    { /* truncated value shall not change the sign
	 and the sign-extended value must be the same.  */
      bfd_signed_vma t = value;
      int msb;
      t >>= howto->bitsize - 1;
      msb = t & 1;
      t >>= 1;

      /* workaround glibc build issue (math/libm_pic.a(e_powf.os)).  */
      if (ELFNN_R_TYPE (rel->r_info) == R_RISCV_SUB6
	  || ELFNN_R_TYPE (rel->r_info) == R_RISCV_SUB8)
	return bfd_reloc_ok;

      if ((t != (bfd_signed_vma)-1) || (msb != 1))
	return bfd_reloc_overflow;
    }
			    ...
			    asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	case R_RISCV_ADD64:
	  {
	    bfd_vma old_value = reloexpr.is_expression
		? reloexpr.expression
		: bfd_get (howto->bitsize, input_bfd,
			   contents + rel->r_offset);
