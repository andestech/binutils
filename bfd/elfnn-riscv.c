...
static void andes_relocate_ict_table (struct bfd_link_info *, bfd *);
static void andes_complement_ict_symbols (struct bfd_link_info *info);
static bool
...
static void
riscv_elf_execit_import_table (bfd *abfd ATTRIBUTE_UNUSED, struct bfd_link_info *info);
static int
...
print_tablejump_entries(riscv_table_jump_htab_t *table_jump_htab)
{
  ...
  for (unsigned int z = 0; z < 32 && table_jump_htab->tbj_indexes[z] != 0; z ++)
    printf ("\tindex=%d, sym name=%s, address=0x%08lx, savings=%d\n",
...
riscv_use_table_jump (struct bfd_link_info *info)
{
  ...
  riscv_parse_subset_t riscv_rps_ld_out =
	{&subsets, _bfd_error_handler, &xlen, NULL, false, STATE_DEFAULT, false};
			...
			asection *sec, const Elf_Internal_Rela *relocs)
{
  ...
  if (! is_inited) { /*Andes */
    ...
    andes->set_table_jump = (unsigned int) determine;
  ...
  for (rel = relocs; rel < relocs + sec->reloc_count; rel++)
	...
	case R_RISCV_ICT_64:
	  if (!riscv_elf_update_ict_hash_table (abfd, sec, h, rel))
	    return false;
	  break;
	/* } Andes */
...
riscv_elf_size_dynamic_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  andes_ld_options_t *andes = &htab->andes;
  struct elf_link_hash_entry *jvt_sym = elf_link_hash_lookup (
      elf_hash_table (info), RISCV_TABLE_JUMP_BASE_SYMBOL, false, false, true);
  if (!jvt_sym || jvt_sym->root.type == bfd_link_hash_undefined)
    {
  ...
  /* for ict patch program that main program didn't enable Zcmt */
  else if (jvt_sym->root.u.def.section == bfd_abs_section_ptr
	   && jvt_sym->root.u.def.value == 0)
    andes->set_table_jump = false;
  /* for ict patch program that main program enable Zcmt */
  else if (andes->set_table_jump && !andes->table_jump_import_file)
    {
      (*_bfd_error_handler)
	(_("warning: failed to find import file of table jump. "
	   "If this is a patch program for ict, please disable "
	   "Zcmt or export and import table jump.\n"));
      andes->set_table_jump = false;
    }
		    ...
		    bfd_byte *contents)
{
  ...
  switch (ELFNN_R_TYPE (rel->r_info))
    ...
    case R_RISCV_TLS_GOT_HI20:
    case R_RISCV_TLS_GD_HI20:
    case R_RISCV_ICT_HI20:	 /* Andes */
    case R_RISCV_PCREL_ICT_HI20: /* Andes */
	...
	*(endp) = 0;
    ...
    /* in case linker option "--mno-relax-align".  */
    case R_RISCV_ALIGN_BTB:
      {
	andes_ld_options_t *andes = &execit.htab->andes;
	return andes->set_relax_align ? bfd_reloc_dangerous : bfd_reloc_ok;
      }
    /* } Andes */
			    ...
			    asection **local_sections)
{
  ...
  if (!is_ict_fini && !find_imported_ict_table && nds_ict_sta.list_len)
    {
      andes_complement_ict_symbols (info);
      andes_relocate_ict_table (info, output_bfd);
    }
  is_ict_fini = true;
  /* } Andes */
  ...
  for (rel = relocs; rel < relend; rel++)
	  ...
	  && h->def_regular)
	{
	  ...
	  && input_section == bfd_get_section_by_name (input_bfd,
						       ANDES_ICT_SECTION))
	{
	  /* Don't need to check the mixed ict cases for the
	     imported ict table at the second link-time.  */
	       ...
	       && r_type != R_RISCV_CALL_ICT && r_type != R_RISCV_ICT_64)
	{
	  (*_bfd_error_handler)
	    (_("%pB: Warning: a mixed indirect call function \'%s\' "
	...
	case R_RISCV_ICT_64:
	  {
	    andes_ict_hash_t *hash;
	    struct bfd_link_hash_entry *ict_base;
	    int ict_entry_size;

	    hash = (andes_ict_hash_t *)
	      bfd_hash_lookup (&nds_ict_sta.indirect_call_table,
	    ...
	    relocation = ((ict_base->u.def.value
			  ...
			  + (hash->entry->index * ict_entry_size));
	...
	/* in case linker option "--mno-relax-align".  */
	case R_RISCV_ALIGN_BTB:
	  r = andes->set_relax_align ? bfd_reloc_dangerous : bfd_reloc_ok;
	  break;
	/* } Andes */
...
riscv_merge_arch_attr_info (bfd *ibfd, char *in_arch, char *out_arch)
{
  ...
  riscv_parse_subset_t riscv_rps_ld_in =
    {&in_subsets, _bfd_error_handler, &xlen_in, NULL, false, STATE_LINK, enabled_execit};
  riscv_parse_subset_t riscv_rps_ld_out =
    {&out_subsets, _bfd_error_handler, &xlen_out, NULL, false, STATE_LINK, enabled_execit};
  /* } Andes */
...
riscv_merge_attributes (bfd *ibfd, struct bfd_link_info *info)
{
  ...
  if (!elf_known_obj_attributes_proc (obfd)[0].i)
    {
	 ...
	 we need to store the information of ict_model here.  */
      for (out_attr_list = elf_other_obj_attributes (obfd)[OBJ_ATTR_PROC];
	   out_attr_list; out_attr_list = out_attr_list->next)
	{
  ...
  for (in_attr_list = elf_other_obj_attributes (ibfd)[OBJ_ATTR_PROC];
       in_attr_list; in_attr_list = in_attr_list->next)
    {
      for (out_attr_list = elf_other_obj_attributes (obfd)[OBJ_ATTR_PROC];
	   out_attr_list; out_attr_list = out_attr_list->next)
	{
	  if (in_attr_list->tag == out_attr_list->tag)
		...
		case Tag_RISCV_ict_version:
		  if (in_attr_list->attr.i != out_attr_list->attr.i)
		    {
		      _bfd_error_handler
			(_("error: %pB: conflicting ict version %d, "
			   "the output ict version is %d."),
			 ibfd, in_attr_list->attr.i, out_attr_list->attr.i);
		      result = false;
		    }
		  break;
		case Tag_RISCV_ict_model:
		  if (strcmp (in_attr_list->attr.s,
			...
			(_("error: %pB: conflicting ict model %s, "
			 ...
			 ibfd, in_attr_list->attr.s, out_attr_list->attr.s);
		      result = false;
		    }
  ...
  if (result)
    {
      riscv_subset_list_t subsets = {NULL, NULL, NULL, NULL};
      unsigned xlen;
  ...
  for (i = 0; i < sec->reloc_count; i++)
    {
      ...
      /* { Andes */
      if (ELFNN_R_TYPE (rel->r_info) == R_RISCV_BRANCH
	  || ELFNN_R_TYPE (rel->r_info) == R_RISCV_RVC_BRANCH
	  || ELFNN_R_TYPE (rel->r_info) == R_RISCV_10_PCREL)
	nsta.prev_branch_rel = rel;
      /* } Andes */
	...
	continue;
      ...
      /* { Andes */
      if (nsta.opt->b35102 || nsta.opt->b35102_detector)
	{
	  if (is_b35102_pattern(abfd, sec, link_info, rel))
	    {
	      bfd_byte *contents = elf_section_data (sec)->this_hdr.contents;
	      rel->r_info = ELFNN_R_INFO (0, R_RISCV_NONE);
	      bfd_putl32 (MATCH_ADDI, contents + rel->r_offset);
	      continue;
	    }

	  if (!_riscv_relax_delete_bytes (abfd, sec, rel->r_offset, rel->r_addend,
					  link_info, NULL, 0, sec->size))
	    return false;

	  if (rel->r_user & REL_R_USER_FLG_RELAX)
	    {
	      rel->r_offset -= 4;
	      rel->r_addend = 0;
	      rel->r_user ^= REL_R_USER_FLG_RELAX;
		       ...
		       bool rvc)
{
  ...
  unsigned int limit = is_jump ? 1 + 0 + tbj_htab->tbljt_count
		       ...
		       bool rvc)
{
  ...
  if (!is_table_jump)
      ...
      if (link_info->relax_pass == PASS_ZCE_TABLE_JUMP_COLLECT)
	{
	  ...
	  int benefit = 8 - 2; /* auipc + jalr => cm.jalt */
      ...
      if (link_info->relax_pass == PASS_ZCE_TABLE_JUMP_APPLY)
	{
	  if (((auipc ^ MATCH_TABLE_JUMP) & MASK_CM_JALT) == 0)
		...
		riscv_elf_hash_table (link_info);
	      andes_ld_options_t *andes = &htab->andes;
	      if (!andes->table_jump_import_file)
		rel->r_info = ELFNN_R_INFO (ELFNN_R_SYM (rel->r_info),
					    R_RISCV_TABLE_JUMP);
	      else
		rel->r_info = ELFNN_R_INFO (0, R_RISCV_NONE);
	      *again = true;
	      return _riscv_relax_delete_bytes (abfd, sec, rel->r_offset + 2, 6,
						link_info, pcgp_relocs, 0, sec->size);
	    }
	  return true;
	}
    }
  ...
  if (rvc)
  ...
  else if (VALID_JTYPE_IMM (foff))
    {
      ...
      (rel + 1)->r_user |= REL_R_USER_FLG_RELAX;
    }
  else
    {
...
/* presume gp is XLEN/8 aligned. */
static bfd_vma set_max_alignment (bfd_vma max_alignment,
			   struct bfd_link_info *link_info,
			   bfd_vma gp,
			   unsigned gp_align,
			   asection *sec)
{
	unsigned xlen_divide_8 = ARCH_SIZE >> 3;
	if (riscv_has_subset (link_info, "xandes")
	    && (gp & (xlen_divide_8 - 1)) == 0
	    && (gp_align >= xlen_divide_8
		|| sec == bfd_abs_section_ptr))
	  return max_alignment > xlen_divide_8 ? max_alignment - xlen_divide_8 : 0;
		      ...
		      bool rvc)
{
  ...
  if (!undefined_weak && gp)
	...
	max_alignment = set_max_alignment (MAX (max_alignment, sym_align), link_info,
					   gp, gp_align, h->u.def.section);
      }
    }
  ...
  if (link_info->relax_pass == PASS_ZCE_TABLE_JUMP_APPLY)
      ...
      if (((jal ^ MATCH_TABLE_JUMP) & MASK_CM_JALT) == 0)
	{
	  ...
	  if (!andes->table_jump_import_file)
	    ...
	    rel->r_info = ELFNN_R_INFO (0, R_RISCV_NONE);
	  *again = true;
	  return _riscv_relax_delete_bytes (abfd, sec,
		  rel->r_offset + 2, 2, link_info, pcgp_relocs, 0, sec->size);
	}
      return true;
    }
...
static void
riscv_estimate_table_jump_optimal_count (riscv_table_jump_args *args, int zcmt_force_table_jump)
{
  unsigned int i;
  riscv_table_jump_htab_t *tbj_htab = args->htab;
  int *acc = tbj_htab->accumulation;
  int *sav = tbj_htab->savings;
  int block = 64 / RISCV_ELF_WORD_BYTES; /* entry count per 64-bytes.  */
  int entry_size = zcmt_force_table_jump ? 0 : RISCV_ELF_WORD_BYTES;
...
riscv_table_jump_profiling (riscv_table_jump_htab_t *table_jump_htab,
    riscv_table_jump_args *args, int zcmt_force_table_jump)
{
  int buf[256];
  ...
  riscv_record_table_jump_index (
	  ...
	  args);
  riscv_estimate_table_jump_optimal_count (args, zcmt_force_table_jump); /* cm.jt */
  ...
  riscv_record_table_jump_index (
	  ...
	  args);
  riscv_estimate_table_jump_optimal_count (args, zcmt_force_table_jump); /* cm.jalt */

  /* determine which scheme to apply.  */
  /* 3 types: cm.jt | cm.jalt | cm.jt + cm.jalt  */
  if (!zcmt_force_table_jump && table_jump_htab->tbljalt_count)
    {
...
_bfd_riscv_relax_lui_gp_insn (bfd *abfd, asection *sec, asection *sym_sec,
			      ...
			      bool rvc ATTRIBUTE_UNUSED);
static bool
_bfd_riscv_relax_pc_gp_insn (bfd *abfd, asection *sec, asection *sym_sec,
			     struct bfd_link_info *link_info,
			     Elf_Internal_Rela *rel,
			     bfd_vma symval,
			     bfd_vma max_alignment,
			     ...
			     bool rvc ATTRIBUTE_UNUSED);
static bool
_bfd_riscv_relax_align_btb (bfd *abfd, asection *sec, asection *sym_sec,
			    struct bfd_link_info *link_info,
			    Elf_Internal_Rela *rel,
			    bfd_vma symval,
			    bfd_vma max_alignment ATTRIBUTE_UNUSED,
...
static bool
riscv_record_table_jump_entry (htab_t p, bfd_vma address, unsigned int index);
  ...
  while (rel != end && rel->r_offset == offset)
    {
      ...
      if (type == R_RISCV_NO_RVC_REGION_BEGIN
	  || (type == R_RISCV_RELAX_REGION_BEGIN
	      && (rel->r_addend & R_RISCV_RELAX_REGION_NO_16_BIT_FLAG)))
	result = 1;
      else if (type == R_RISCV_NO_RVC_REGION_END
...
static int
riscv_enable_table_jump (Elf_Internal_Rela *rel, Elf_Internal_Rela *end)
{
  int type, result = -1;
  bfd_vma offset = rel->r_offset;
  ...
  while (rel != end && rel->r_offset == offset)
    {
      type = ELFNN_R_TYPE (rel->r_info);
      if (type == R_RISCV_RELAX_REGION_BEGIN
	  && rel->r_addend & R_RISCV_RELAX_REGION_NO_TABLE_JUMP_FLAG)
	result = 1;
      else if (type == R_RISCV_RELAX_REGION_END
	       && rel->r_addend & R_RISCV_RELAX_REGION_NO_TABLE_JUMP_FLAG)
	result = 0;
      rel = rel + 1;
    }
			  ...
			  bool *again)
{
  ...
  *again = false;
  ...
  if (is_init == 0)
      ...
      if (nsta.opt == NULL)
	{
	  ...
	  riscv_has_subset (info, "xandesnewcodense") ||
      ...
      if (andes->gp_relative_insn)
	{
      ...
      if (andes->set_table_jump && !htab->andes.table_jump_import_file)
	{
  ...
  switch (info->relax_pass)
      ...
      if (sec == execit.final_sec)
	{
      ...
      return true;
    case PASS_ANDES_GP_1 ... PASS_ZCE_TABLE_JUMP_APPLY:
      break;
    case PASS_ALIGN_ORG:
  ...
  if (info->relax_pass == PASS_ZCE_TABLE_JUMP_COLLECT)
      ...
      if (info->relax_trip == 0)
	{
	  if (andes->table_jump_import_file)
		...
		return true;
	      ...
	      FILE *import_file = andes->table_jump_import_file;
	      riscv_table_jump_htab_t *tjhtab = table_jump_htab;
	      unsigned int tbljt_count, tbljalt_count;
	      unsigned char count_buf[2];

	      int r = fread (count_buf, sizeof (unsigned char),
			     2, import_file);
	      BFD_ASSERT (r == 2 && count_buf[0] < 33 && count_buf[1] < 225);
	      tbljt_count = count_buf[0];
	      tbljalt_count = count_buf[1];
	      if (tbljalt_count == 0)
		{
		  for (i = 0; i < tbljt_count; ++i)
		    {
		      r = fread (tjhtab->tbj_indexes + i, sizeof (uintNN_t),
				 1, import_file);
		      BFD_ASSERT (r == 1);
		      riscv_record_table_jump_entry (tjhtab->tbljt_htab,
						     tjhtab->tbj_indexes[i],
						     tbljt_count - i);
		    }
		}
	      else
		{
		  for (i = 32 - tbljt_count; i < 32; ++i)
		    {
		      r = fread (tjhtab->tbj_indexes + i, sizeof (uintNN_t),
				 1, import_file);
		      BFD_ASSERT (r == 1);
		  ...
		  for (i = 32; i < 32 + tbljalt_count; ++i)
		    {
		      r = fread (tjhtab->tbj_indexes + i, sizeof (uintNN_t),
				 1, import_file);
		      BFD_ASSERT (r == 1);
		      riscv_record_table_jump_entry (tjhtab->tbljalt_htab,
						     tjhtab->tbj_indexes[i],
						     i + 1);
		    }
		}
	      fclose (import_file);
	      table_jump_htab->end_idx = 32 + tbljalt_count;
	      table_jump_htab->tbljt_count = tbljt_count;
	      table_jump_htab->tbljalt_count = tbljalt_count;
	      return true;
	    }
      ...
      else if (info->relax_trip == 1)
	{
	  *again = true;
	  ...
	  riscv_table_jump_profiling (table_jump_htab, &args, htab->zcmt_force_table_jump);
	  return true;
	}
      ...
      else if (info->relax_trip == 2)
	{
	  ...
	  if (andes->table_jump_import_file)
	    {
	      if (!(table_jump_htab->tbljt_count
		    || table_jump_htab->tbljalt_count))
		return true;
	    }
	  else if (!(table_jump_htab->tbljt_count
		     || table_jump_htab->tbljalt_count)
		   && tjsec->size > 0 && !htab->zcmt_force_table_jump)
	    {
	      ...
	      return _riscv_relax_delete_bytes (owner, tjsec, 0, tjsec->size, info, NULL, 0, tjsec->size);
	    }
      ...
      else if (info->relax_trip == 3)
	{
	  ...
	  /* Trim unused slots.  */
	  if ((trimmed_bytes > 0) &&
	      !_riscv_relax_delete_bytes (table_jump_htab->tablejump_sec_owner,
				table_jump_htab->tablejump_sec,
				used_bytes, trimmed_bytes, info, NULL, 0, table_jump_htab->tablejump_sec->size))
	    return false;
  ...
  table_jump = andes->set_table_jump;
  ...
  for (i = 0; i < sec->reloc_count; i++)
	...
	{
	case 1: rvc = 0; break;
	case 0: rvc = elf_elfheader (abfd)->e_flags & EF_RISCV_RVC; break;
	default: break;
	}
      switch (riscv_enable_table_jump (rel, relocs_end))
	{
	case 1: table_jump = 0; break;
	case 0: table_jump = andes->set_table_jump; break;
	default: break;
	}
	  ...
	  if (info->relax_trip == 0 || info->relax_trip == 2)
	    {
		  ...
		  && (type == R_RISCV_CALL
		      || type == R_RISCV_CALL_PLT))
		relax_func = _bfd_riscv_relax_call;
	      else if (andes->set_relax_jump && type == R_RISCV_JAL)
	  ...
	  if (andes->set_relax_lui
	    ...
	    relax_func = _bfd_riscv_relax_lui_gp_insn;
		   ...
		   && (type == R_RISCV_PCREL_HI20
		       || type == R_RISCV_PCREL_LO12_I
		       || type == R_RISCV_PCREL_LO12_S))
	    relax_func = _bfd_riscv_relax_pc_gp_insn;
	  else
	    continue;
  ...
  /* { Andes */
  if (info->relax_pass == PASS_ANDES_GP_1)
 ...
 fail:
  ...
  if (info->relax_pass == PASS_ANDES_GP_2 + 1)
...
riscv_elf_object_p (bfd *abfd)
{
  ...
  /* Build the ict hash table to store all global symbols attached
     with ICT suffix.  */
  if (!nds_ict_sta.is_init)
    {
      riscv_elf_ict_init ();
      nds_ict_sta.is_init = 1;
    }
  /* } Andes */
...
riscv_final_link (bfd * abfd, struct bfd_link_info * info)
{
  ...
  if (andes->set_table_jump
      && tjhtab->tablejump_sec)
    {
      ...
      if (andes->table_jump_export_file != NULL)
	{
	  export_file = fopen (andes->table_jump_export_file, "wb");
	  if (export_file == NULL)
	    {
	      (*_bfd_error_handler)
		(_("Warning: cannot open the exported .riscv.jvt %s."),
		   andes->table_jump_export_file);
	    }
	}
      ...
      if (export_file)
	fprintf (export_file, "%c%c", (char)tjhtab->tbljt_count,
		 (char)tjhtab->tbljalt_count);
      ...
      if (tjhtab->tbljalt_count == 0)
	{
	  ...
	  if (export_file)
	    fwrite (buf, sizeof (uintNN_t), tjhtab->tbljt_count, export_file);
	}
      else
	{
	  ...
	  for (int i=32; i < (32 + tjhtab->tbljalt_count); ++i)
	    buf[i - bias] = tjhtab->tbj_indexes[i];
	  if (export_file)
	    fwrite (buf, sizeof (uintNN_t),
		    tjhtab->tbljt_count + tjhtab->tbljalt_count, export_file);
	}
      if (export_file)
	fclose (export_file);
  ...
  if (nsta.opt->b35102_detector)
    {
      char filename[64];
      time_t t = time (NULL);
      strftime (filename, sizeof (filename),
		"b35102_ld_detector_%Y%m%d_%H%M%S.log", localtime (&t));
      FILE *fp = fopen (filename, "w");
      if (!fp)
	(*_bfd_error_handler) (_("Warning: cannot open %s."), filename);
      else
	{
	  fprintf (fp, "Count of nop: %u\n", nsta.b35102_nop_count);
	  fclose (fp);
	}
    }
  /* } Andes */
...
static bfd_vma
_bfd_riscv_get_max_alignment_gp_insn (asection *sec, bfd_vma gp)
{
  unsigned int max_alignment_power = 0;
  asection *o;
...
static int
andes_min_p2align (Elf_Internal_Rela *rel, asection *sec, bfd_vma symval)
{
...
static int
andes_relax_gp_insn (bfd *abfd, asection *sec, asection *sym_sec,
		     Elf_Internal_Rela *rel, bfd_vma symval,
		     riscv_pcgp_hi_reloc *hi, bfd_vma gp, bfd_vma guard_size)
{
  bfd_byte *contents = elf_section_data (sec)->this_hdr.contents;
  ...
  int worst_p2alig = andes_min_p2align (rel, sym_sec, symval);
  int sym = hi->hi_sym;
  ...
  if ((insn & MASK_ADDI) == MATCH_ADDI && VALID_GPTYPE_LB_IMM (bias))
  ...
  else if ((insn & MASK_LH) == MATCH_LH && VALID_GPTYPE_LB_IMM (bias)
	   && worst_p2alig > 0)
    {
  ...
  else if ((insn & MASK_LHU) == MATCH_LHU && VALID_GPTYPE_LB_IMM (bias)
	   && worst_p2alig > 0)
    {
      rel->r_info = ELFNN_R_INFO (sym, R_RISCV_LGP17S1);
      rel->r_addend = hi->hi_addend;
	   ...
	   && bias <= (1 << 18) - 1 && worst_p2alig > 1)
    {
      ...
      insn = (insn & (OP_MASK_RD << OP_SH_RD)) | MATCH_NDS_LWUGP;
	   ...
	   && bias <= (1 << 19) - 1 && worst_p2alig > 2 && ARCH_SIZE == 64)
    {
  ...
  else if ((insn & MASK_SH) == MATCH_SH && VALID_GPTYPE_SB_IMM (bias)
	   && worst_p2alig > 0)
    {
...
record_and_find_relax_gp_syms (asection *sec, Elf_Internal_Sym *lsym,
			       struct elf_link_hash_entry *h, int record)
{
			      ...
			      bool rvc ATTRIBUTE_UNUSED)
{
  ...
  if ((data_start == 0) || (sec_addr (sym_sec) < data_start))
    guard_size += link_info->relro ? andes->set_relax_page_size * 2
  ...
  if (hh->u.def.section->output_section == sym_sec->output_section
  ...
  else
    {
      ...
      max_alignment = _bfd_riscv_get_max_alignment_gp_insn (sec, gp);
    }

  /* Since we do not have grouping mechanism like v3, we may
  ...
  switch (ELFNN_R_TYPE (rel->r_info))
	...
	hi.hi_addend = rel->r_addend;
	if (!andes_relax_gp_insn (abfd, sec, sym_sec, rel,
				  symval, &hi, gp, guard_size))
	  record_and_find_relax_gp_syms (sym_sec, isym, h, 1);
      }
      break;

    case R_RISCV_HI20:
      if (!record_and_find_relax_gp_syms (sym_sec, isym, h, 0))
	{
...
static bool
riscv_record_pcgp_hi_reloc_ext (riscv_pcgp_relocs *p, bfd_vma hi_sec_off,
				bfd_vma hi_addend, bfd_vma hi_addr,
				unsigned hi_sym, asection *sym_sec,
				bool undefined_weak, Elf_Internal_Rela *rel)
{
  bool rz = riscv_record_pcgp_hi_reloc (p, hi_sec_off, hi_addend,
					hi_addr, hi_sym, sym_sec,
					undefined_weak);
  if (rz)
     p->hi->rel = rel;
  return rz;
...
static bool
_bfd_riscv_relax_pc_gp_insn (bfd *abfd,
			     asection *sec,
			     asection *sym_sec,
			     struct bfd_link_info *link_info,
			     Elf_Internal_Rela *rel,
			     ...
			     bool rvc ATTRIBUTE_UNUSED)
{
  ...
  switch (ELFNN_R_TYPE (rel->r_info))
    ...
    case R_RISCV_PCREL_HI20:
	riscv_record_pcgp_hi_reloc_ext (pcgp_relocs,
					rel->r_offset,
					rel->r_addend,
					symval,
					ELFNN_R_SYM (rel->r_info),
  ...
  if (undefined_weak || sym_sec->output_section == bfd_abs_section_ptr
      || (sym_sec != bfd_abs_section_ptr
	  && elf_section_flags (sym_sec) & SHF_EXECINSTR))
    {
      riscv_record_pcgp_lo_reloc (pcgp_relocs, hi_sec_off);
  ...
  if (hi_reloc.rel && hi_reloc.rel->r_user & REL_R_USER_FLG_JVT)
...
riscv_elf_ict_hash_newfunc (struct bfd_hash_entry *entry,
			    ...
			    const char *string ATTRIBUTE_UNUSED)
{
  const size_t sz_entry = sizeof (andes_ict_hash_t);
  ...
  if (entry == NULL)
    {
      entry = (void *) bfd_hash_allocate (table, sz_entry);
...
{
  ict_model = 0xffffffff;
  if (!bfd_hash_table_init_n
      (&nds_ict_sta.indirect_call_table, riscv_elf_ict_hash_newfunc,
       sizeof (andes_ict_hash_t), 1023))
    (*_bfd_error_handler)
				 ...
				 const Elf_Internal_Rela *rel)
{
  ...
  if (h)
      ...
      if (entry == NULL)
	{
	  ...
	  andes_ict_entry_t *it = andes_ict_list_update_symbol (h);
	  entry->entry = it;
	  nds_ict_sta.hash_entries++;
	}
    }
  else
    {
...
/* non-invoked ict symbols are not collected in ict list.
   complement ict table for thest symbols.
*/
static void
andes_complement_ict_symbols (struct bfd_link_info *info)
{
  bfd *abfd;
  asection *sec;

  /* find ict table section.  */
  ...
  /* check if any ict entry patched.  */
...
/* any ict table entry should be with a symbol `h',
   if the symbol is from linker script, then the `vma' is the VMA of symbol.
   otherwise the VMA shall be resolved from symbol `h'.  */

static void
andes_relocate_ict_table (struct bfd_link_info *info, bfd *output_bfd)
{
  bfd *input_bfd;
  asection *sec;
  bfd_byte *contents = NULL;
  ...
  ict_sym = bfd_link_hash_lookup (info->hash, "_INDIRECT_CALL_TABLE_BASE_",
				  false, false, false);
  ict_base = ict_sym->u.def.value + sec_addr (ict_sym->u.def.section);
  ...
  while (head)
	  ...
	  || (h->root.type == bfd_link_hash_undefined))
	{
	  ...
	  if (h->root.ldscript_def == 1 && !(head->flags & ICT_FLG_VMA))
	    {
	      h->root.u.def.value = head->vma - secaddr;
	      head->flags |= ICT_FLG_VMA;
	    }
	  ...
	  if (ict_model == 0)
	    ...
	    {
	      (*_bfd_error_handler) (_("Linker: Unknown ICT model.\n"));
	      return;
	    }
	  ...
	  insn = bfd_get (howto->bitsize, output_bfd,
			  contents + (index * ict_entry_size));
	  insn = (insn & ~howto->dst_mask) | (relocation & howto->dst_mask);
      ...
      else
	{
	  ...
	  (*_bfd_error_handler)
	    (_("Linker: Undefined ICT symbol `%s'!\n"), name);
	  BFD_ASSERT (0);
	}
			      ...
			      struct elf_link_hash_entry *h ATTRIBUTE_UNUSED)
{
  ...
  if (!nsta.check_start_export_sym)
      ...
      if (p)
	{
	  int i = 0;
	  struct bfd_link_hash_entry *ict_base =
	    bfd_link_hash_lookup (info->hash, "_INDIRECT_CALL_TABLE_BASE_",
				  false, false, false);
	  bfd_vma ict_base_vma =
	    ict_base->u.def.value + sec_addr (ict_base->u.def.section);
  ...
  while ((*irel) != NULL && (*irel) < irelend && (*off) == (*irel)->r_offset)
	...
	case R_RISCV_RELAX_REGION_BEGIN:
	  ...
	  is_no_execit = ((*irel)->r_addend & R_RISCV_RELAX_REGION_NO_EXECIT_FLAG
			  || (*irel)->r_addend & R_RISCV_RELAX_REGION_NO_16_BIT_FLAG);
		  ...
		  if (ELFNN_R_TYPE ((*irel)->r_info) == R_RISCV_RELAX_REGION_BEGIN)
		    { /* nested region detection.  */
		      ...
		      next_is_no_execit = ((*irel)->r_addend & R_RISCV_RELAX_REGION_NO_EXECIT_FLAG
					   || (*irel)->r_addend & R_RISCV_RELAX_REGION_NO_16_BIT_FLAG);
		      next_is_inner_loop = execit_loop_aware
					   && ((*irel)->r_addend & R_RISCV_RELAX_REGION_IMLOOP_FLAG
					       && !((*irel)->r_addend & R_RISCV_RELAX_REGION_NO_EXECIT_FLAG));
		      if (next_is_no_execit || next_is_inner_loop)
			nested_execit_depth++;
		      else
			continue;
  ...
  /* Integer Register-Immediate Operations.  */
  else if ((insn16 & MASK_C_ADDI) == MATCH_C_ADDI)
    {
      ...
      *insn32 = RISCV_ITYPE (ADDI, rd, rd, imm); /* addi rd, rd, nzimm  */
	   ...
	   && (insn16 & MASK_C_ADDIW) == MATCH_C_ADDIW)
    {
      /* CI format to I-TYPE.  */
      int rd = (insn16 >> 7) & 0x1f;
      imm = EXTRACT_CITYPE_IMM (insn16);
      *insn32 = RISCV_ITYPE (ADDIW, rd, rd, imm); /* addiw rd, rd, imm  */
    }
  ...
  /* Integer Register-Register Operations.  */
  ...
  else if ((insn16 & MASK_C_SUB) == MATCH_C_SUB)
    {
      ...
      *insn32 = RISCV_RTYPE (SUB, rd, rd, rs2); /* sub rd, rd, rs2  */
	   ...
	   && (insn16 & MASK_C_ADDW) == MATCH_C_ADDW)
    {
      /* CS format to R-TYPE.  */
      int rd = 8 + ((insn16 >> OP_SH_CRS1S) & OP_MASK_CRS1S);
      int rs2 = 8 + ((insn16 >> OP_SH_CRS2S) & OP_MASK_CRS2S);
      *insn32 = RISCV_RTYPE (ADDW, rd, rd, rs2); /* addw rd, rd, rs2  */
    }
  else if ((ARCH_SIZE == 64)
	   && (insn16 & MASK_C_SUBW) == MATCH_C_SUBW)
    {
      /* CS format to R-TYPE.  */
      int rd = 8 + ((insn16 >> OP_SH_CRS1S) & OP_MASK_CRS1S);
      int rs2 = 8 + ((insn16 >> OP_SH_CRS2S) & OP_MASK_CRS2S);
      *insn32 = RISCV_RTYPE (SUBW, rd, rd, rs2); /* subw rd, rd, rs2  */
    }
  else
    {
			...
			bfd_vma offset, bfd_vma end)
{
...

  {
	...
	&& target_align_check_branch_range (abfd, sec, insn16_off, end,
					    2, info))
		  ...
		  || rtype == R_RISCV_CALL_PLT)))
	{
	  ...
	  if (rtype == R_RISCV_JAL &&
	    ...
	    return rz;
		   ...
		   || rtype == R_RISCV_PCREL_HI20 || rtype == R_RISCV_CALL_PLT)
		  ...
		  || rtype == R_RISCV_CALL_PLT)
		{
		  ...
		  ctx->ie.bias = RISCV_CONST_HIGH_PART (bias);
		}
	...
	{ /* skip future check for some relocs  */
	  ...
	  if ((rta == R_RISCV_HI20 && rtb == R_RISCV_HI20)
	      || ((rta == R_RISCV_CALL || rta == R_RISCV_PCREL_HI20
		   || rta == R_RISCV_CALL_PLT)
		  && (rtb == R_RISCV_CALL || rtb == R_RISCV_PCREL_HI20
		      || rtb == R_RISCV_CALL_PLT)))
	    return b;
	  ...
	  while (p)
	    {
	      ...
	      _riscv_relax_delete_bytes (qabfd->abfd, qsec->sec, offset,
					 q->size, info, NULL, 0, qsec->sec->size);
			    ...
			    bool rvc ATTRIBUTE_UNUSED)
{
  ...
  return _riscv_relax_delete_bytes (abfd, sec, rel->r_offset + filled,
				    rel->r_addend, info, NULL, 0, sec->size);
}

static bool
riscv_record_table_jump_entry (htab_t p, bfd_vma address, unsigned int index)
{
  riscv_table_jump_htab_entry entry = { .address = address, .index = index };
  riscv_table_jump_htab_entry **slot =
    (riscv_table_jump_htab_entry **)htab_find_slot (p, &entry, INSERT);
  if (slot == NULL)
    return false;

  *slot =
    (riscv_table_jump_htab_entry *)bfd_zmalloc
      (sizeof (riscv_table_jump_htab_entry));
  if (*slot == NULL)
    return false;
  **slot = entry;
...
static bool
is_alu_insn(bfd *abfd, bfd_byte *contents, bfd_vma offset)
{
  if (IS_RVC_INSN(*(contents + offset)))
    {
      uint16_t insn16 = bfd_get_16 (abfd, contents + offset);
      if ((insn16 & MASK_C_ADDI) == MATCH_C_ADDI
	  || (insn16 & MASK_C_SLLI) == MATCH_C_SLLI
	  || (insn16 & MASK_C_SRLI) == MATCH_C_SRLI
	  || (insn16 & MASK_C_SRAI) == MATCH_C_SRAI
	  || (insn16 & MASK_C_ANDI) == MATCH_C_ANDI
	  || (insn16 & MASK_C_LI) == MATCH_C_LI
	  || (insn16 & MASK_C_LUI) == MATCH_C_LUI
	  || (insn16 & MASK_C_MV) == MATCH_C_MV
	  || (ARCH_SIZE != 32 && (insn16 & MASK_C_ADDIW) == MATCH_C_ADDIW))
	return true;
    }
  else
    {
      uint32_t insn32 = bfd_get_32 (abfd, contents + offset);
      if ((insn32 & MASK_ADDI) == MATCH_ADDI
	  || (insn32 & MASK_SLTI) == MATCH_SLTI
	  || (insn32 & MASK_SLTIU) == MATCH_SLTIU
	  || (insn32 & MASK_XORI) == MATCH_XORI
	  || (insn32 & MASK_ORI) == MATCH_ORI
	  || (insn32 & MASK_ANDI) == MATCH_ANDI
	  || (insn32 & MASK_SLLI) == MATCH_SLLI
	  || (insn32 & MASK_SRLI) == MATCH_SRLI
	  || (insn32 & MASK_SRAI) == MATCH_SRAI
	  || (insn32 & MASK_LUI) == MATCH_LUI
	  || (insn32 & MASK_AUIPC) == MATCH_AUIPC
	  || (insn32 & MASK_NDS_ADDIGP) == MATCH_NDS_ADDIGP
	  || (insn32 & MASK_NDS_BFOS) == MATCH_NDS_BFOS
	  || (insn32 & MASK_NDS_BFOZ) == MATCH_NDS_BFOZ
	  || (ARCH_SIZE != 32 && ((insn32 & MASK_ADDIW) == MATCH_ADDIW
				  || (insn32 & MASK_SLLIW) == MATCH_SLLIW
				  || (insn32 & MASK_SRLIW) == MATCH_SRLIW
				  || (insn32 & MASK_SRAIW) == MATCH_SRAIW)))
	return true;
    }
  return false;
}

#define NEXT_INSN_OFFSET(contents, offset) \
  (offset) + ((*((contents) + (offset)) & 3) == 3 ? 4 : 2)
...
{
  if (!(nsta.opt->b35102 || nsta.opt->b35102_detector)
      || !nsta.prev_branch_rel)
    return false;
  Elf_Internal_Rela *branch_rel = nsta.prev_branch_rel;
  Elf_Internal_Shdr *symtab_hdr = &elf_tdata (abfd)->symtab_hdr;
  struct riscv_elf_link_hash_table *htab = riscv_elf_hash_table (link_info);
  asection *sym_sec;
  bfd_vma symval;
  /* Get the value of the symbol referred to by the reloc.  */
  if (ELFNN_R_SYM (branch_rel->r_info) < symtab_hdr->sh_info)
    {
      /* A local symbol.  */
      Elf_Internal_Sym *isym = ((Elf_Internal_Sym *) symtab_hdr->contents
				+ ELFNN_R_SYM (branch_rel->r_info));
      if (isym->st_shndx == SHN_UNDEF)
	return false;
      else
	{
  ...
  else
    {
      unsigned long indx;
      struct elf_link_hash_entry *h;
      indx = ELFNN_R_SYM (branch_rel->r_info) - symtab_hdr->sh_info;
      h = elf_sym_hashes (abfd)[indx];
      while (h->root.type == bfd_link_hash_indirect
	     || h->root.type == bfd_link_hash_warning)
	h = (struct elf_link_hash_entry *) h->root.u.i.link;
	...
	return false;
	  /* This line has to match the check in riscv_elf_relocate_section
	     in the R_RISCV_CALL[_PLT] case.  */
      if (bfd_link_pic (link_info) && h->plt.offset != MINUS_ONE)
	{
	  sym_sec = htab->elf.splt;
	  symval = h->plt.offset;
	}
  ...
  bfd_vma branch_offset = branch_rel->r_offset;
  bfd_vma target_offset = symval + branch_rel->r_addend;
  if (target_offset <= branch_offset
      || target_offset - branch_offset > 12)
    return false;
  bfd_byte *contents = elf_section_data (sec)->this_hdr.contents;
  bfd_vma insn1_offset = NEXT_INSN_OFFSET(contents, branch_rel->r_offset);
  if (insn1_offset == sec->size)
    return false;
  bfd_vma insn2_offset = NEXT_INSN_OFFSET(contents, insn1_offset);
  if (insn2_offset == sec->size)
    return false;
  bfd_vma insn3_offset = NEXT_INSN_OFFSET(contents, insn2_offset);
  if (insn3_offset != target_offset
      || (rel->r_offset != insn1_offset
	  && rel->r_offset != insn2_offset))
    return false;

  bfd_vma insn_offset =
    rel->r_offset == insn1_offset ? insn2_offset : insn1_offset;
  if (!is_alu_insn(abfd, contents, insn_offset))
    return false;
  if (nsta.opt->b35102_detector)
    nsta.b35102_nop_count++;
  if (nsta.opt->b35102)
