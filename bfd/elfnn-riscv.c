...
static void andes_execit_build_itable (struct bfd_link_info *link_info);
static void andes_execit_relocate_itable (struct bfd_link_info *);
...
andes_extend_irel(Elf_Internal_Rela *irel, int subtype, andes_irelx_t **list);
			...
			asection *sec, const Elf_Internal_Rela *relocs)
{
  ...
  if (! is_inited) { /*Andes */
    is_inited = TRUE;
    memset (&nsta, 0, sizeof (andes_linker_state_t));
    nsta.htab = htab;
    nsta.opt = &htab->option;
  } /* Andes */
...
riscv_record_pcrel_hi_reloc (riscv_pcrel_relocs *p, bfd_vma addr,
			     bfd_vma value, int type, bfd_boolean absolute)
{
			    ...
			    asection **local_sections)
{
  ...
  if (execit.is_itable_relocated == 0)
    { /* init itable before relocating ITE.  */
      if (htab->option.target_optimization & RISCV_RELAX_EXECIT_ON)
	...
	case R_RISCV_ANDES_TAG:
	  {
		...
		/* update offset and addend for itable relocation.  */
		       ...
		       bfd_boolean rvc)
{
  ...
  if ((sym_sec->output_section != sec->output_section) &&
      (!table->option.set_relax_cross_section_call))
    return TRUE;
			      ...
			      bfd_boolean rvc ATTRIBUTE_UNUSED)
{
  ...
  if ((data_start == 0) || (sec_addr (sym_sec) < data_start))
    guard_size += link_info->relro ? htab->option.set_relax_page_size * 2
				   : htab->option.set_relax_page_size;
  ...
  if ((ARCH_SIZE == 64))
    ...
    gp_align = 4;
  if (htab->option.gp_relative_insn
      && ((gp % gp_align) != 0))
    {
			...
			bfd_boolean rvc)
{
  ...
  if (htab->option.target_aligned && rvc
  ...
  if (insn != 0xffffffff
  ...
  else
    {
      ...
      if (align_btb && htab->option.avoid_btb_miss
	  && riscv_relax_check_BTB_miss (abfd, sec, rel->r_offset,
			  ...
			  bfd_boolean *again)
{
  ...
  if (is_init == 0)
    { /* init states  */
      is_init = 1;
      /* page size  */
      if (htab->option.set_relax_page_size == 0)
	htab->option.set_relax_page_size = ELF_MAXPAGESIZE;
      /* execit state  */
  ...
  *again = FALSE;
  ...
  if (info->relax_pass > 0)
    { /* skip pass 1~4 if -mno-relax
       * skip pass 5~6, 9 if --mno-execit
       * no skip pass 8 for all (execit, gp-relative)
       */
      if (info->disable_target_specific_optimizations
	   && (info->relax_pass != 4) && (info->relax_pass < 7))
	return TRUE;
      if ((htab->option.target_optimization & RISCV_RELAX_EXECIT_ON) == 0)
	{
  ...
  if (!execit.is_init && info->relax_pass == 5)
    {
	  ...
	  (elf_elfheader (output_bfd)->e_flags & EF_RISCV_RVC))
	{
	  ...
	  if (htab->option.execit_import_file)
	    {
	      execit.is_built = 1;
  ...
  /* Check relax_pass and then do the corresponding relaxation.
    ...
    Pass 8: Relax special relocations (for detection, redo in relocation)
  ...
  */
  switch (info->relax_pass)
      ...
      if (sec == execit.final_sec)
	{
	  ...
	  if (htab->option.update_execit_table && !execit.is_replace_again)
	    {
	...
	{ /* finalize itable size  */
	  ...
	  if ((htab->option.execit_import_file == NULL) ||
	      htab->option.keep_import_execit ||
	      htab->option.update_execit_table)
	    {
	      asection *table_sec;
  ...
  for (i = 0; i < sec->reloc_count; i++)
	...
	{
	  if (htab->option.set_relax_call
	      && (type == R_RISCV_CALL
		  || type == R_RISCV_CALL_PLT))
	    relax_func = _bfd_riscv_relax_call;
	  else if (htab->option.set_relax_lui
		   && !htab->option.gp_relative_insn
		   && (type == R_RISCV_HI20
		       || type == R_RISCV_LO12_I
		       || type == R_RISCV_LO12_S))
	    relax_func = _bfd_riscv_relax_lui;
	  else if (htab->option.set_relax_pc
		   && !bfd_link_pic(info)
		   && (type == R_RISCV_PCREL_HI20
		       || type == R_RISCV_PCREL_LO12_I
		       || type == R_RISCV_PCREL_LO12_S))
	    { /* bug #23695 PCREL GP-instruction relaxation support.  */
	      relax_func = (htab->option.gp_relative_insn)
			    ? andes_relax_pc_gp_insn
			    : _bfd_riscv_relax_pc;
	    }
	  else if (htab->option.set_relax_tls_le
	...
	relax_func = _bfd_riscv_relax_delete;
	       ...
	       && htab->option.set_relax_align
	       && (type == R_RISCV_ALIGN
		   || type == R_RISCV_ALIGN_BTB))
	relax_func = _bfd_riscv_relax_align;
      else if (info->relax_pass == 8 && type == R_RISCV_ANDES_TAG)
	{
	  #if 0 /* it's not safe to do relocation here!
		   as VMA has not determined yet.  */
	  if (ELFNN_R_SYM (rel->r_info) == R_RISCV_EXECIT_ITE)
	  ...
	  else if (ELFNN_R_SYM (rel->r_info) == MISC_GPREL_SUBTYPE_FLX)
	    { /* FLX rd, *sym(gp) */
	      andes_irelx_t *irel_ext =
		(andes_irelx_t *) rel->r_addend;
	      rel->r_info = irel_ext->saved_irel.r_info;
	  ...
	  else if (ELFNN_R_SYM (rel->r_info) == MISC_GPREL_SUBTYPE_FSX)
	    { /* FSX rd, *sym(gp) */
	      andes_irelx_t *irel_ext =
		(andes_irelx_t *) rel->r_addend;
	      rel->r_info = irel_ext->saved_irel.r_info;
...
fail:
  if (relocs != data->relocs)
    free (relocs);
  if ((info->relax_pass == 1) && htab->option.gp_relative_insn)
			      ...
			      struct elf_link_hash_entry *h ATTRIBUTE_UNUSED)
{
  ...
  sym_ld_script = table->option.sym_ld_script;
  if (!sym_ld_script)
    return TRUE;
			    ...
			    ATTRIBUTE_UNUSED)
{
  ...
  htab = riscv_elf_hash_table (info);
  sym_ld_script = htab->option.sym_ld_script;
...
static int andes_execit_hi20_collect_multiple (hi20_context_t *ctx, int rtype)
{
  ...
  for (p = ctx->p->next; p; pp = p, p = p->next)
      ...
      if ((count > pre_count) && (count > (entries * 2)))
	{
      ...
      /* try merge previous 'small' group of uhi20.  */
      if (found && rtype == R_RISCV_HI20 && ctx->uhi20s)
	{
	  hi20_group_t *grp = ctx->uhi20s;
	  while (grp)
	    {
      ...
      /* try merge previous 'small' group of hi20.  */
...
static int andes_execit_estimate_hi20_post (hi20_context_t *ctx)
{
  ...
  if (rtype == R_RISCV_HI20
      && p->ie.entries == 0) /* merge one-entry group doestn't gain.  */
    {
      if (ctx->uhi20s == NULL)
	{
      ...
      else
	{
	  ...
	  while (grp)
	    {
	      if ((grp->tail->next == p)
		  && (p->ie.bias - grp->head->ie.bias) < SZ_4K)
		{
		  grp->tail = p;
		  grp->insts += 1;
		  break;
...
andes_execit_build_itable (struct bfd_link_info *info)
{
  ...
  limit = htab->option.execit_limit;
  /* odd old definition (v5_toolmisc_test)  */
  if (htab->option.execit_import_file &&
      htab->option.update_execit_table &&
      htab->option.execit_limit >= 0)
  ...
  /* Write EXECIT candidates into .exec.itable. We will
     ...
     in the andes_execit_relocate_itable.  */
  ...
  for (index = 0; index < total; index++)
    execit.itable_array[index].index = index;
  index = 0;
  for (p = execit.rank_list;
       p && index < limit;
       p = p->next)
...
riscv_elf_execit_import_table (bfd *abfd ATTRIBUTE_UNUSED, struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  htab = riscv_elf_hash_table (info);
  execit_import_file = htab->option.execit_import_file;
  rewind (htab->option.execit_import_file);
  ...
  if (htab->option.update_execit_table
      && htab->option.execit_limit != -1
      && (execit_import_number + htab->option.execit_limit) > EXECIT_HW_ENTRY_MAX)
    (*_bfd_error_handler)
...
andes_execit_relocate_itable (struct bfd_link_info *info)
{
  ...
  if (opt->execit_export_file != NULL)
    {
      export_file = fopen (opt->execit_export_file, "wb");
      if (export_file == NULL)
	{
	  (*_bfd_error_handler)
	    (_("Warning: cannot open the exported .exec.itable %s."),
	     opt->execit_export_file);
	}
    }

  /* TODO: Maybe we should close the export file here, too.  */
  if (opt->execit_import_file && !opt->update_execit_table)
    return;

  itable_sec = riscv_elf_execit_get_section (info->input_bfds);
  if (itable_sec == NULL)
    {
...
riscv_elf_execit_check_insn_available (uint32_t insn,
				       struct riscv_elf_link_hash_table *htab)
{
  andes_ld_options_t *opt = &htab->option;
  ...
  if (!opt->execit_flags.rvp)
    { /* RVP is excluded.  */
      if (major == MATCH_OP_P)
	return FALSE;
    }

  if (!opt->execit_flags.fls)
...
andes_execit_render_hash (execit_context_t *ctx)
{
  ...
  if (irel)
      ...
      if (rtype == R_RISCV_ANDES_TAG)
	{
      ...
      if ((!nsta.opt->execit_flags.noji && rtype == R_RISCV_JAL)
	  || (!nsta.opt->execit_flags.nols
		  ...
		  || stype != TAG_NONE))
	  || (!nsta.opt->execit_flags.no_auipc
	      && (rtype == R_RISCV_CALL || rtype == R_RISCV_PCREL_HI20)))
	{
	  unsigned long r_symndx = ELFNN_R_SYM (irel->r_info);
	  if (r_symndx < symtab_hdr->sh_info)
...
{
  /* to track R_RISCV_CALL pals (why?) */
  ...
  htab = riscv_elf_hash_table (info);
  execit_loop_aware = htab->option.execit_loop_aware;
	      ...
	      while ((*irel) && (*irel) < irelend && (is_no_execit || is_inner_loop))
		{
		  ...
		  if (ELFNN_R_TYPE ((*irel)->r_info) == R_RISCV_RELAX_REGION_BEGIN)
		    { /* nested region detection.  */
		      bfd_boolean next_is_no_execit, next_is_inner_loop;
		      nested_execit_depth++;
			...
			next_is_inner_loop = (*irel)->r_addend & R_RISCV_RELAX_REGION_LOOP_FLAG;
...
riscv_final_link (bfd * abfd, struct bfd_link_info * info)
{
  ...
  while (TRUE) /* once */
    {
      asection *itable_sec = riscv_elf_execit_get_section (info->input_bfds);
      bfd_byte *contents = NULL;
      if (itable_sec == NULL)
	break;
  ...
  bfd_boolean rvc ATTRIBUTE_UNUSED)
{
  ...
  /* reserved one page size in worst case
     or two (refer to riscv_relax_lui_to_rvc)  */
  if ((data_start == 0) || (sec_addr (sym_sec) < data_start))
    guard_size += info->relro ? htab->option.set_relax_page_size * 2
				   : htab->option.set_relax_page_size;
		     ...
		     bfd_signed_vma bias, int sym, asection *sym_sec)
{
  ...
  if ((*insn & MASK_ADDI) == MATCH_ADDI && VALID_GPTYPE_LB_IMM (bias))
    ...
    { /* only for non-relative relocations  */
	...
	relocation -= pc;
	  ...
	  if (!is_found)
	      ...
	      else
		{
		  ...
		  BFD_ASSERT (t->index == index);
		  *t = execit.itable_array[execit_index];
		  t->index = index; /* restore index.  */
		  t->next = 0;
		  t->relocation = hi20;
		  t->inf_ie = ie;
		  ie->itable_index = index;
      ...
      /* apply relocation  */
      bfd_byte *contents = elf_section_data (sec)->this_hdr.contents;
      uint16_t insn16 = (uint16_t)EXECIT_INSN | ENCODE_RVC_EXECIT_IMM (item->index << 2);
  ...
  bfd_boolean undefined_weak ATTRIBUTE_UNUSED)
{
  ...
  else if (ELFNN_R_TYPE (rel->r_info) == R_RISCV_LO12_S)
    {
