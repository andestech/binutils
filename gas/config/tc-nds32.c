...
do_pseudo_ls_bhw (int argc ATTRIBUTE_UNUSED, char *argv[], int pv)
{
  ...
  if (exp.X_op != O_symbol)
    {
      as_bad (_("l.[bhw][s] and s.[bhw] must use with symbol. '%s'"),
	      argv[argc]);
      return;
    }
...
nds32_set_elf_flags_by_insn (struct nds32_asm_insn * insn)
{
  ...
  while (new_flags)
	...
	case NASM_ATTR_IFC_EXT:
	  ...
	  break;
	case NASM_ATTR_DSP2_ISAEXT:
	  if (nds32_dsp_ext)
	    {
	      nds32_elf_flags |= E_NDS32_HAS_DSP2_INST;
	      skip_flags |= NASM_ATTR_DSP2_ISAEXT;
	    }
...
nds32_elf_append_relax_relocs (const char *key, void *value)
{
  ...
  while (pattern_now && pattern_now->opcode)
    {
			  ...
			  void *xxx ATTRIBUTE_UNUSED)
{
  ...
  for (fixp = seginfo->fix_root; fixp; fixp = fixp->fx_next)
    if (!fixp->fx_done)
      break;
  if (!fixp && !enable_relax_ex9 && !enable_relax_ifc
      && !verbatim && (!ict_exist || ict_flag == ICT_NONE))
    return;
  ...
  if (!enable_relax_relocs)
    X_add_number |= R_NDS32_RELAX_ENTRY_DISABLE_RELAX_FLAG;
  else
    {
	...
	X_add_number |= R_NDS32_RELAX_ENTRY_EX9_FLAG;
      if (enable_relax_ifc)
	X_add_number |= R_NDS32_RELAX_ENTRY_IFC_FLAG;
      if (verbatim)
	X_add_number |= R_NDS32_RELAX_ENTRY_VERBATIM_FLAG;
      if (ict_exist && ict_flag == ICT_SMALL)
	X_add_number |= R_NDS32_RELAX_ENTRY_ICT_SMALL;
      else if (ict_exist && ict_flag == ICT_LARGE)
	X_add_number |= R_NDS32_RELAX_ENTRY_ICT_LARGE;
    }
  if (optimize)
    X_add_number |= R_NDS32_RELAX_ENTRY_OPTIMIZE_FLAG;
  if (optimize_for_space)
    X_add_number |= R_NDS32_RELAX_ENTRY_OPTIMIZE_FOR_SPACE_FLAG;

  fixP = fix_at_start (fragP, 0, abs_section_sym, X_add_number,
