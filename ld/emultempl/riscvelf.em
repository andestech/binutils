...
static andes_ld_options_t andes =
{
  ...
  .execit_auipc_entry = 2, /* 2 for worst cases (b28185) */
  .execit_flags = { 0 },
  .update_execit_table = 0,
  .keep_import_execit = 0,
  .execit_loop_aware = 0,
  .execit_jal_over_2m = 0,
  .table_jump_import_file = NULL,
  .table_jump_export_file = NULL,
  .b35102 = 0,
  .b35102_detector = 0,
...
riscv_elf_after_check_relocs (void)
{
  ...
  if (htab->andes.set_table_jump && !htab->andes.table_jump_import_file)
    {
...
enum risccv_opt
{
  ...
  /* { Andes */
  /* NOTE: Do not add values >= 400 in this enum.
     Another enum starts from 400, so the value range must not overlap.  */
  OPTION_EXPORT_SYMBOLS,

  /* These are only for internal usage and debugging.  */
  /* Relax lui to nds v5 gp insn.  */
  OPTION_GP_RELATIVE_INSN,
  OPTION_NO_GP_RELATIVE_INSN,
  /* Alignment relaxations.  */
  OPTION_NO_RELAX_ALIGN,
  OPTION_NO_TARGET_ALIGNED,
  OPTION_AVOID_BTB_MISS,
  OPTION_NO_AVOID_BTB_MISS,
  /* Disable specific relaxation.  */
  OPTION_NO_RELAX_LUI,
  OPTION_NO_RELAX_PC,
  OPTION_NO_RELAX_CALL,
  OPTION_NO_RELAX_TLS_LE,
  OPTION_RELAX_CROSS_SECTION_CALL,
  OPTION_NO_RELAX_CROSS_SECTION_CALL,
  OPTION_NO_WORKAROUND,
  OPTION_TRUNCATION_CHECK,
  OPTION_NO_TRUNCATION_CHECK,
  OPTION_EXPORT_TABLE_JUMP,
  OPTION_IMPORT_TABLE_JUMP,
  OPTION_MB35102,
  OPTION_MB35102_DETECTOR,

  /* These are only available for Andes internal options.  */
#if defined RISCV_ANDES_INTERNAL_OPTIONS
  OPTION_ZCMT_FORCE_JUMP_TABLE,
  OPTION_NO_OPT_TABLE_JUMP,
  OPTION_OPT_TABLE_JUMP,
#endif

  /* These are only available to EXEC.IT.  */
#if defined RISCV_EXECIT_EXT
  OPTION_EXECIT_TABLE,
  OPTION_NO_EXECIT_TABLE,
  OPTION_EXPORT_EXECIT,
  OPTION_IMPORT_EXECIT,
  OPTION_KEEP_IMPORT_EXECIT,
  OPTION_UPDATE_EXECIT,
  OPTION_EXECIT_LIMIT,
  OPTION_EXECIT_LOOP,
  OPTION_EXECIT_NO_JI,
  OPTION_EXECIT_NO_LS,
  OPTION_EXECIT_NO_REL,
  OPTION_EXECIT_AUIPC,
  OPTION_NO_EXECIT_AUIPC,
  OPTION_EXECIT_JAL,
  OPTION_NO_EXECIT_JAL,
  OPTION_EXECIT_JAL_OVER_2M,
  OPTION_NO_EXECIT_JAL_OVER_2M,
  OPTION_EXECIT_FLS,
  OPTION_NO_EXECIT_FLS,
  OPTION_EXECIT_RVV,
  OPTION_NO_EXECIT_RVV,
  OPTION_EXECIT_XDSP,
  OPTION_NO_EXECIT_XDSP,
  OPTION_EXECIT_RVP,
  OPTION_NO_EXECIT_RVP,
  OPTION_NEXECIT_OP,
  OPTION_EXECIT_AUIPC_ENTRY,
  OPTION_EXECIT_JUMP,
  OPTION_EXECIT_NO_JUMP,
#endif

  /* These are only for lld internal usage and not affected for bfd.  */
  OPTION_LLD_COMPATIBLE,
  OPTION_BEST_GP,
  OPTION_EXECIT_OPT_DATA,
  OPTION_EXECIT_OPT_RODATA,
  OPTION_EXECIT_SEPERATE_CALL,
  OPTION_RELAX_GP_TO_RODATA,
  OPTION_DEBUG_EXECIT_LIMIT,
  OPTION_NO_EXECIT_LUI,
  OPTION_ALLOW_INCOMPATIBLE_ATTR,
  OPTION_FULL_SHUTDOWN,
  /* } Andes */
};
'
...
PARSE_AND_LIST_LONGOPTS=${PARSE_AND_LIST_LONGOPTS}'
    ...
    {"mtruncation-check", no_argument, NULL, OPTION_TRUNCATION_CHECK},
    {"mno-truncation-check", no_argument, NULL, OPTION_NO_TRUNCATION_CHECK},
    {"mexport-table-jump", required_argument, NULL, OPTION_EXPORT_TABLE_JUMP},
    {"mimport-table-jump", required_argument, NULL, OPTION_IMPORT_TABLE_JUMP},
    {"mb35102", no_argument, NULL, OPTION_MB35102},
    {"mb35102-detector", no_argument, NULL, OPTION_MB35102_DETECTOR},

    /* These are specific options for Zcmt support.  */
#if defined RISCV_ANDES_INTERNAL_OPTIONS
...
/* { Andes */
  ...
  --m[no-]relax-cross-section-call\n\
                              Enable/Disable cross-section relaxations\n\
  --mno-workaround            Disable all workarounds\n\
"));

#if defined RISCV_ANDES_INTERNAL_OPTIONS
fprintf (file, _("\
    --zcmt-force-table-jump     Force to generate table jump instructions\n\
"));
#endif

#if defined RISCV_EXECIT_EXT
  fprintf (file, _("\
  --m[no-]execit              Enable/Disable link-time EXEC.IT relaxation\n\
  --mexport-execit=FILE       Export .exec.itable after linking\n\
  --mimport-execit=FILE       Import .exec.itable for EXEC.IT relaxation\n\
  ...
  --m[no-]execit-fls          Enable/Disable EXEC.IT conversion for floating load/store instructions\n\
  --m[no-]execit-rvv          Enable/Disable EXEC.IT conversion for RVV instructions\n\
  --m[no-]execit-xdsp         Enable/Disable EXEC.IT conversion for XDSP instructions\n\
  --m[no-]execit-auipc        Enable/Disable EXEC.IT conversion for AUIPC instruction\n\
  --m[no-]execit-jal          Enable/Disable EXEC.IT conversion for JAL instruction\n\
  --m[no-]execit-jal-over-2mib\n\
                              Enable/Disable EXEC.IT conversion for JAL instruction over the first 2MiB page of text section\n\
  --m[no-]execit-jump         Enable/Disable EXEC.IT conversion for jump instructions\n\
"));
  ...
  if (var)
    {
      fprintf (file, _("\
  --mexecit-rvp               Enable EXEC.IT conversion for RVP instructions\n\
"));
    }
#endif
/* } Andes */
...
PARSE_AND_LIST_ARGS_CASES=${PARSE_AND_LIST_ARGS_CASES}'
    ...
    case OPTION_EXPORT_TABLE_JUMP:
      if (!optarg)
	einfo (_("Missing file for --mexport-table-jump=<file>.\n"));
      andes.table_jump_export_file = xstrdup (optarg);
      break;

    case OPTION_IMPORT_TABLE_JUMP:
      if (!optarg)
	einfo (_("Missing file for --mimport-table-jump=<file>.\n"));
      andes.table_jump_import_file = fopen (optarg, "rb+");
      if (andes.table_jump_import_file == NULL)
	einfo (_("ERROR %P%F: cannot open table jump import file %s.\n"), optarg);
      break;
    ...
    case OPTION_TRUNCATION_CHECK:
    case OPTION_NO_TRUNCATION_CHECK:
...
gld${EMULATION_NAME}_after_allocation (void)
{
  ...
  /* { Andes */
  /* Add a symbol for linker script to check the max size.  */
