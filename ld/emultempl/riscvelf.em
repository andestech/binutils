...
D4_RCSC=1 # default on for both elf/linux tool-chain
...
static andes_ld_options_t option =
{
  .sym_ld_script = NULL,
  .set_relax_align = 1,
  .target_aligned = 1,
  ...
  .execit_flags = {.no_auipc = 1}, /* disable auipc by default */
...
riscv_elf_after_open (void)
{
  ...
  if ((option.target_optimization & RISCV_RELAX_EXECIT_ON)
      && (option.execit_import_file == NULL
	  || option.keep_import_execit
	  || option.update_execit_table))
    {
...
#define OPTION_RELAX_PAGE_SIZE		(OPTION_INTERNAL_BASELINE + 14)
...
#define OPTION_EXECIT_AUIPC		(OPTION_EXECIT_BASELINE + 17)
#define OPTION_NO_EXECIT_AUIPC		(OPTION_EXECIT_BASELINE + 18)
#define OPTION_EXECIT_LUI		(OPTION_LLD_COMPATIBLE_BASELINE + 19)
#define OPTION_NO_EXECIT_LUI		(OPTION_LLD_COMPATIBLE_BASELINE + 20)
#define OPTION_EXECIT_AUIPC_ENTRY		(OPTION_EXECIT_BASELINE + 21)
#endif
...
/* These are specific options for EXECIT support.  */
  ...
  { "mexecit-lui", no_argument, NULL, OPTION_EXECIT_LUI},
...
PARSE_AND_LIST_ARGS_CASES='
  case OPTION_EXPORT_SYMBOLS:
    ...
    if(strcmp (optarg, "-") == 0)
      option.sym_ld_script = stdout;
    else
      {
	option.sym_ld_script = fopen (optarg, FOPEN_WT);
	if(option.sym_ld_script == NULL)
	einfo (_("%P%F: cannot open map file %s: %E.\n"), optarg);
      }
    break;

  case OPTION_GP_RELATIVE_INSN:
    option.gp_relative_insn = 1;
    break;
  case OPTION_NO_GP_RELATIVE_INSN:
    option.gp_relative_insn = 0;
    break;
  case OPTION_NO_RELAX_ALIGN:
    option.set_relax_align = 0;
    break;
  case OPTION_NO_TARGET_ALIGNED:
    option.target_aligned = 0;
    break;
  case OPTION_AVOID_BTB_MISS:
    option.avoid_btb_miss = 1;
    break;
  case OPTION_NO_AVOID_BTB_MISS:
    option.avoid_btb_miss = 0;
    break;
  case OPTION_NO_RELAX_LUI:
    option.set_relax_lui = 0;
    break;
  case OPTION_NO_RELAX_PC:
    option.set_relax_pc = 0;
    break;
  case OPTION_NO_RELAX_CALL:
    option.set_relax_call = 0;
    break;
  case OPTION_NO_RELAX_TLS_LE:
    option.set_relax_tls_le = 0;
    break;
  case OPTION_RELAX_CROSS_SECTION_CALL:
    option.set_relax_cross_section_call = 1;
    break;
  case OPTION_NO_RELAX_CROSS_SECTION_CALL:
    option.set_relax_cross_section_call = 0;
    break;
  case OPTION_NO_WORKAROUND:
    option.set_workaround = 0;
    break;
  case OPTION_RELAX_PAGE_SIZE:
    if (!optarg)
      einfo (_("Missing value for --mrelax-page-size.\n"));
    else
      option.set_relax_page_size = strtol(optarg, NULL, 0);
    break;

#if defined RISCV_EXECIT_EXT
  case OPTION_EX9_TABLE:
    if (option.execit_limit == -1
	|| option.execit_limit > 512)
    option.execit_limit = 512;
    /* FALL THROUGH.  */
  case OPTION_EXECIT_TABLE:
    option.target_optimization |= RISCV_RELAX_EXECIT_ON;
    break;
  ...
  case OPTION_EXPORT_EXECIT:
    if (!optarg)
      ...
      option.execit_export_file = optarg;
  ...
  case OPTION_IMPORT_EXECIT:
    ...
    option.execit_import_file = fopen (optarg, "rb+");
    if(option.execit_import_file == NULL)
      einfo (_("ERROR %P%F: cannot open execit import file %s.\n"), optarg);
    break;
  case OPTION_KEEP_IMPORT_EXECIT:
    option.keep_import_execit = 1;
    break;
  case OPTION_UPDATE_EXECIT:
    option.update_execit_table = 1;
    break;
  case OPTION_EXECIT_LIMIT:
    if (optarg)
      {
	if (option.execit_limit != -1
	    && atoi (optarg) > option.execit_limit)
	  einfo (_("Warning: the value of execit_limit (%d) is larger "
		   "than the current setting (%d)\n"),
		 atoi (optarg), option.execit_limit);
	else
	  option.execit_limit = atoi (optarg);

	if (option.execit_limit > 1024 || option.execit_limit < 0)
	  {
  ...
  case OPTION_EXECIT_LOOP:
    option.execit_loop_aware = 1;
    break;
  case OPTION_EXECIT_NO_JI:
    option.execit_flags.noji = 1;
    break;
  case OPTION_EXECIT_NO_LS:
    option.execit_flags.nols = 1;
    break;
  ...
  case OPTION_EXECIT_AUIPC:
    option.execit_flags.no_auipc = 0;
    break;
  case OPTION_NO_EXECIT_AUIPC:
    option.execit_flags.no_auipc = 1;
    break;
  case OPTION_EXECIT_LUI:
    option.execit_flags.no_lui = 0;
    break;
  case OPTION_NO_EXECIT_LUI:
    option.execit_flags.no_lui = 1;
    break;
  case OPTION_EXECIT_RVV:
    option.execit_flags.rvv = 1;
    break;
  case OPTION_EXECIT_RVP:
    option.execit_flags.rvp = 1;
    break;
  case OPTION_EXECIT_FLS:
    option.execit_flags.fls = 1;
    break;
  case OPTION_EXECIT_XDSP:
    option.execit_flags.xdsp = 1;
    break;
  case OPTION_EXECIT_AUIPC_ENTRY:
    if (!optarg)
      ...
      {
	option.execit_auipc_entry = atoi (optarg);
	if(option.execit_auipc_entry == 0)
	  {
	    option.execit_auipc_entry = 1;
