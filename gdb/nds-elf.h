...
static void NEC_buf_init(char *buf, unsigned int len)
{
    nec_buf = buf;
    nec_buf_len = len;
    buf[0] = '\0';
...
static void NEC_snprintf(char *buf, size_t size, const char *str, ...)
{
#define TEMP_SZ 100
    int width;
    size_t len = 0;
...
static inline void NEC_print(ELF_Fail_Type type, const char *name, const char *cpu, const char *elf, const char *error_message)
{
    switch (type)
    {
    case EFT_NONE:
        NEC_output("\t | %9s | %9s | %21s\n", cpu, elf, name);
        break;
    case EFT_WARNING:
        NEC_output("\t?| %9s | %9s | %21s Warning: %s\n", cpu, elf, name, error_message);
        break;
    case EFT_ERROR:
        NEC_output("\t!| %9s | %9s | %21s Error: %s\n", cpu, elf, name, error_message);
        break;
    }
}

static inline bool NEC_check_bool(ELF_Fail_Type type, const char *isa, bool cpu, bool elf)
{
    bool code;
    ...
    NEC_print(type, isa, NEC_MSG_ISA[cpu], NEC_MSG_ISA[elf], "Not supported by CPU");
    return code;
}
...
static const char *base_isas[BASE_ISA_COUNT] = {"RV32E", "RV32I", "RV64I", "RV128I"};
...
static const char *riscv_x_extensions[] = {"xandesv", "Xv5-", "xdsp", "xefhw", "xnexecit"};

static const char *riscv_c_extensions[] = {"Zca", "Zcb", "Zcd", "Zcf", "Zcmp", "Zcmt"};
...
static const char *riscv_zilsd_extensions[] = {"Zilsd", "Zcmlsd", "Zclsd"};
...
static void init_elf_info(void)
{
    memset(&nds_info, 0, sizeof(nds_info));
}

static void set_riscv_base_isa_info(int index, int major, int minor)
{
...
static void get_riscv_base_isa_info(int *index, int *major, int *minor)
{
...
static void set_riscv_ext_info(int index, int major, int minor)
{
    if (index >= EXT_COUNT)
        return;
...
static int get_riscv_ext_info_i(int i, bool *use, int *major, int *minor)
{
    if (i >= EXT_COUNT)
        return -1;
...
static void set_riscv_x_ext_info(const char *ext, int major, int minor)
{
    ...
    else if (strncmp(ext, riscv_x_extensions[X_EXT_NEXECIT], 8) == 0)
    {
        nds_info.x_ext_use[X_EXT_NEXECIT] = true;
        nds_info.x_ext_major[X_EXT_NEXECIT] = major;
        nds_info.x_ext_minor[X_EXT_NEXECIT] = minor;
    }
}

/* Get non-standard extension info by index. */
static int get_riscv_x_ext_info_i(int i, bool *use, int *major, int *minor)
{
    if (i >= X_EXT_COUNT)
        return -1;
...
static bool elf_use_ext_i(int i, ext_type type)
{
    bool use = false;
    int major, minor;
    switch (type)
...
static bool cpu_support_std_ext(reg_t misa, reg_t mmsc_cfg, char ext)
{
...
static bool cpu_support_arch_config(reg_t mrvarch_cfg, const char *ext, bool is_mrvarch_cfg_exist)
{
    ...
    else if ((strncmp(ext, "Zcmlsd", 6) == 0) || (strncmp(ext, "Zclsd", 5) == 0))
    {
...
static bool cpu_support_arch_c_config(reg_t misa, reg_t mrvarch_cfg, const char *ext, bool is_mrvarch_cfg_exist, char *c_other_error_info)
{
    ...
    {
        is_ext_en = ((mrvarch_cfg & 0x8000000) != 0);
    }
    ...
    {
        is_ext_en = ((mrvarch_cfg & 0x40000000) != 0);
    }
    else if (strncmp(ext, "Zcmt", 4) == 0)
    {
...
static bool cpu_support_v5dsp_ext(reg_t mmsc_cfg, reg_t misa)
{
...
static bool cpu_support_v5efhw_ext(reg_t mmsc_cfg, reg_t misa)
{
...
static bool in_buffer(const buf_t buf, const size_t off, const size_t len)
{
...
static int parse_byte(const uint8_t **buf, const uint8_t *end, uint8_t *ret)
{
    if (end - *buf < 1)
...
static int parse_uint32le(const uint8_t **buf, const uint8_t *end, uint32_t *ret)
{
    if (end - *buf < 4)
        die("Truncated ELF attribute section");
...
static int parse_ntbs(const uint8_t **buf, const uint8_t *end, const char **ret)
{
...
static int parse_uleb128(const uint8_t **buf, const uint8_t *end, uint64_t *ret)
{
    uint64_t shift = 0;
    uint64_t value = 0;
    while (*buf < end)
...
static int parse_decimal(const char **s, int *ret)
{
    if (!isdigit(**s))
...
static int parse_march_name_end(const char **s)
{
    const char *p_str = *s;
    /* "m2p0_", "zic64b1p0_" */
    while (1) {
...
static int parse_riscv_isa_version(const char **s, int *p_major, int *p_minor)
{
...
static int parse_riscv_base_isa(const char **s)
{
    int major = 0, minor = 0;
...
static const char *pStr_Tag_RISCV_arch = NULL;

static int parse_riscv_isa_string(const char *s)
{
    pStr_Tag_RISCV_arch = s;
    ...
    while (*s != '\0')
    {
        ...
        else if (e == ext && (tolower(*e) != 'x' && tolower(*e) != 'z' && tolower(*e) != 's'))
        ...
        else
        {
            ...
            else if (strncasecmp(s, "xnexecit", 8) == 0)
            {
                print_indent("%.8s ", s);
                s += 8;
                strncpy(x_ext, riscv_x_extensions[X_EXT_NEXECIT], 9);
            }
            // b-ext
            ...
            else if (strncasecmp(s, "zcb", 3) == 0)
            {
                print_indent("%.3s ", s);
                s += 3;
                type = ext_type::C_EXT;
                nds_info.c_ext_use[C_EXT_ZCB] = true;
            }
            else if (strncasecmp(s, "zcd", 3) == 0)
            {
                print_indent("%.3s ", s);
                s += 3;
                type = ext_type::C_EXT;
            ...
            else if (strncasecmp(s, "zcmp", 4) == 0)
            {
                print_indent("%.4s ", s);
                s += 4;
                type = ext_type::C_EXT;
                nds_info.c_ext_use[C_EXT_ZCMP] = true;
            }
            else if (strncasecmp(s, "zcmt", 4) == 0)
            {
                print_indent("%.4s ", s);
                s += 4;
                type = ext_type::C_EXT;
                nds_info.c_ext_use[C_EXT_ZCMT] = true;
            }
            // cmo-ext
            else if (strncasecmp(s, "zicbom", 6) == 0)
            {
                print_indent("%.6s ", s);
                s += 6;
                type = ext_type::CMO_EXT;
                nds_info.cmo_ext_use[CMO_EXT_ZICBOM] = true;
            }
            else if (strncasecmp(s, "zicbop", 6) == 0)
            {
                print_indent("%.6s ", s);
                s += 6;
                type = ext_type::CMO_EXT;
                nds_info.cmo_ext_use[CMO_EXT_ZICBOP] = true;
            }
            else if (strncasecmp(s, "zicboz", 6) == 0)
            {
                print_indent("%.6s ", s);
                s += 6;
                type = ext_type::CMO_EXT;
                nds_info.cmo_ext_use[CMO_EXT_ZICBOZ] = true;
            }
            // misc-ext
            else if (strncasecmp(s, "svinval", 7) == 0)
            {
                print_indent("%.7s ", s);
                s += 7;
            ...
            else if (strncasecmp(s, "zclsd", 5) == 0)
            {
                print_indent("%.5s ", s);
                s += 5;
                type = ext_type::ZILSD_EXT;
                nds_info.zilsd_ext_use[ZILSD_EXT_ZCLSD] = true;
            }
            else
            {
                const char *end = s;
...
static int parse_legacy_riscv_attributes(const uint8_t *buf, const uint8_t *end)
{
    while (buf < end)
    {
...
static int parse_riscv_attributes(const uint8_t *buf, const uint8_t *end)
{
    while (buf < end)
    {
        // Each attribute is a pair of tag and value. The value can be either a
...
static int is_legacy_riscv_attributes(const uint8_t *buf, const uint8_t *end)
{
    uint64_t tag;
    if (parse_uleb128(&buf, end, &tag) == -1)
        return -1;
    return tag == 4 && end - buf >= 2 && strncasecmp((const char *)buf, "rv", 2) == 0;
}

static int parse_riscv_subsection(const uint8_t *buf, const uint8_t *end)
{
    while (buf < end)
    {
...
static int parse_elf_attribute_section(const uint8_t *buf, const uint8_t *end)
{
    init_elf_info();
...
static int elf_check(void *file_data, unsigned int file_size, CALLBACK_FUNC reg_read_callback, char *buf, unsigned int len)
{
    ...
    Debug_printf("RISC-V attribute section at file offset %#tx with size %#zx\n", sec.data - file.data, sec.size);
    ...
    reg_t CSR_misa;
    reg_t CSR_mmsc_cfg = 0, CSR_mmsc_cfg2 = 0;
    reg_t CSR_mrvarch_cfg = 0;
    reg_t CSR_mrvarch_cfg2 = 0;
    ...
    {
        if (NEC_check_bool(EFT_ERROR, "V5 Extension", cpu_support_v5_ext(CSR_mmsc_cfg), true))
            n_error++;
    }
    ...
    {
        if (NEC_check_bool(EFT_ERROR, "V5 DSP Extension", cpu_support_v5dsp_ext(CSR_mmsc_cfg, CSR_misa), true))
            n_error++;
    }

    if (elf_use_ext_i(X_EXT_FHW, ext_type::X_EXT))
    {
        if (NEC_check_bool(EFT_ERROR, "V5 EFHW Extension", cpu_support_v5efhw_ext(CSR_mmsc_cfg, CSR_misa), true))
            n_error++;
    }

    if (elf_use_ext_i(X_EXT_NEXECIT, ext_type::X_EXT))
    {
        if (NEC_check_bool(EFT_ERROR, "V5 NEXECIT Extension", cpu_support_v5nexeit_ext(CSR_mmsc_cfg, CSR_mmsc_cfg2, is64), true))
            n_error++;
    }
    ...
    if (n_error)
    {
        ...
        NEC_output("Usage error, Consult Andes Toolchains and their compatible Andes cores for the Toolchain-CPU compatibility.\n");
        NEC_output("The Loader Checking can be disabled under Debug Configuration.\n");
    }
    else
